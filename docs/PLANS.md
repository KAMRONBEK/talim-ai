...
| QA-DEFER-HOTSPOT-A11Y | HOTSPOT + DRAG_DROP structured-player keyboard/touch a11y is untested — they are **assessment-builder-only** (image + regions / drag targets), so an unattended run can't reach them without an owner building an image-based assessment. Reachable players (DROPDOWN_CLOZE/MATCHING/ORDERING) all PASS keyboard-a11y (Run 19 C6). | Needs an owner-built HOTSPOT/DRAG_DROP assessment fixture (image upload + region/target config) before the learner-side a11y can be driven; ORDERING's move-button pattern suggests DRAG_DROP likely has a keyboard fallback, but it's unverified. | @KAMRONBEK | 2026-07-14 |
| O82 | Podcast episode `durationSec` (shown in the episode-list rows) under-estimates the real audio by ~10s (episode 1: stored 93s/"1:33" vs the loaded `<audio>.duration` 103s/"1:43", readyState=4 = ground truth; ~11% short). Player time is correct; only the list label is off. | The stored value is derived at synthesis from mp3 **byte-length ≈ ms** (CBR assumption) — VBR/ID3/padding make it drift. True fix = probe the real decoded duration post-synthesis and persist it; a backend `generatePodcast` job change, not a low-risk UI tweak. | @KAMRONBEK | 2026-07-14 |
