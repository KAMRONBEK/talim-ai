...
| O98 | perf | US-XCUT · layout-stability | The sweep failed `layout-stability` on **11 cells** (learner dashboard CLS **0.276**, tenant students **0.272**, limit 0.25). Re-measured live on `/uz/learner/dashboard` with layout-shift attribution: **CLS 0.19 — under the limit**, so it straddles the threshold rather than reproducing on demand. The shift is real though: at t≈312ms a ~178px block is inserted above the fold and pushes `DIV.relative.overflow-hidden` (y 77→255), the stat grid (273→451) and the card below (474→652) down together, then again at t≈328ms — i.e. content mounting after data arrives without reserved space, not font swap. **S3-shaped and non-reproducible above threshold → O, not F** (per §E, only S1/S2 non-repros become flaky Fs). Worth re-measuring against a **production build** before acting: dev-mode CSS/font delivery inflates CLS, so the 0.25 breach may not exist in the artifact users get. **Narrowed R2026-08-25b:** the tenant-students half of this was reproduced 3/3 at 0.289 with a single named cause and split out as **F92** (#44). What remains under O98 is the learner-dashboard family, which still straddles: hand-measured 0.139 (uz-light) and 0.19 last cycle, but 0.316 in the same-cycle sweep — variance, not a stable breach. A **production-build** measurement is still the right next step. Six of the original 11 cells were not product at all: they were redirect cells whose CLS belonged to the destination page, and the probe was fixed in 8a37531c to stop measuring them. | R2026-08-25a | open (low-confidence) |
