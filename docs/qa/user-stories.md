...
| F59 | S2 | US-IND-24 · EC1 | **Quiz generation that returns 0 questions / FAILS spins forever — no FAILED state.** The `Quiz` model has **no `status` field**, so when `generateQuiz.job` throws "No quiz questions generated" (0 Qs) or the job fails, there is nowhere to persist FAILED; `GET /quiz/:id` returns a quiz with `questions: []`. `useQuiz` polls until `questions.length` is truthy (never, on failure), and `QuizCard`/quiz page only branch on `questions.length === 0` → render the "generating…" spinner **indefinitely** with no error/retry. The job already publishes an SSE `quiz.status: FAILED` event, but nothing persists/reads it. **Fix needs a schema migration** (`Quiz.status` + persist READY/FAILED in the job + a failed/retry UI branch + locale strings) → **logged, not auto-applied** (migrations are sensitive on this drift-prone local DB — human review; same discipline as F11/F45/F46). | 🟡 logged | — |
