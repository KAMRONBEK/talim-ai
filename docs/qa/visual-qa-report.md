...
### C2 — Information-dense tutor dashboard · Nodira · FedEx · data-integrity (RCRCRC #2, 7ae7893f)

**Charter:** Explore `/tenant/dashboard` as a skeptical owner shadowing each rendered number back to its backend source, to discover **data-integrity** defects (stat cards ≠ `/tenant/progress` + results). **Done when:** every stat card + class-snapshot + assessment-activity count is reproduced from `/tenant/progress` and `/tenant/assessments/:id/results`, the "avg mastery" fix is verified self-consistent, values persist after a real reload, and console/network are clean.

**Oracle (curl):** `/tenant/progress` → totals{students:5, activeStudents:5, materials:1, avgCoverage:3.875, avgQuizScore:65, atRisk:5}; distribution{lt50:5, rest:0}; weakest topic "Darsga kirish…" coverage 0; roster: only **Test Student One** has mastery (0, assigned 1, active 2 days ago), the other 4 mastery=null.

**🟢 Every stat card reconciles.** Materials **1 · "1 ta tayyor"** ✓; Faol o'quvchilar **5 · "jami 5 ta"** ✓; Xavf ostida **5** ✓ (all lt50); Topshiriqlar **2** ✓ (Written+Game); Bu hafta faol **1 · "5 ta faol o'quvchidan"** ✓ (only Test Student One `isRecentlyActive`).

**🟢 "Avg mastery" fix verified self-consistent (the commit's headline fix).** Card shows **0%**, NOT the `totals.avgCoverage=3.875`. Source (`dashboard/page.tsx:88`) deliberately averages **active students' per-student `mastery`** (the roster figure), excluding `avgCoverage` "which also counts the owner's own reading". masteredStudents = [Test Student One mastery 0] → avg 0 → "0%". Correct: not "—" (1 student has data), not 3.875 (section-coverage incl. owner). The "was mis-averaging quiz scores" fix holds. (Backend curio: `totals.avgQuizScore=65` while no student row has a non-null `avgQuizScore` — not surfaced on the dashboard; noted, not filed.)

**🟢 Class snapshot + assessment-activity match backend.** Distribution 85%+/70-84/50-69/<50 = 0/0/0/**5** ✓; weakest topic "Darsga kirish…" 0% ✓; top-of-class "Test Student One 0%" ✓. Assessment activity: "QA Written Quiz! · 0/1 topshirdi" + "QA Game Quiz · 2/2 topshirdi" — verified vs `/results`: game `learners`=2 both `submitted:true` (2/2 ✓), written 1 learner unsubmitted (0/1 ✓). Recent activity (Test Student One, 2 kun oldin, 0%), messages preview ("Salom! Ertaga matematikadan…" 1 kun oldin · 0 o'qidi · 0 javob), recent materials (YouTube) all match.

**🟢 Depth≥3 + dragnet.** Real `location.reload()` → all values persist; **0 console errors**, every API call **200** (progress/students/assessments/results/messages), no duplicate mutation POSTs (session-sync double-fetch is GETs only). **Charisma spot-check:** 390px viewport → `scrollWidth==clientWidth`, **0 horizontal overflow**, 0 wide elements.

**Oracle:** data-integrity (product self-consistency — rendered = backend) + World (arithmetic). **No F.** **Spotted for bad-neighborhood:** an existing roster test-student named `🎓 Ali <script>alert(1)</script> Очень…Длинное…` (XSS+long-name payload) — dashboard doesn't surface it (not in leaderboard/activity), but the **students page** must escape+truncate it → queued for C3/bad-neighborhood. **Test-data:** none (read-only). **Cells:** tenant.dashboard/info-dense-stats → oracle-verified; pricing/OWNER → oracle-verified (CTA bounce to /tenant/dashboard confirmed, O90).

### C3 — Information-dense student dashboard · Aziza · FedEx · data-integrity + tenant-isolation (RCRCRC #2, 7ae7893f)

**Charter:** Explore `/learner/dashboard` as a student shadowing each rendered number to its backend source AND probing the assigned-only invariant, to discover **data-integrity** + **tenant-isolation** defects. **Done when:** all 4 stat cards + achievements + heatmap + weakest-topics + unread-message banner reconcile with `/learner/progress`+`/learner/summary`, a non-assigned content id is denied, and console is clean on the dashboard itself.

**Oracle (curl):** `/learner/progress` → overallMastery 0, streakDays 0, materialsDone 0, quizzesTaken 1, avgAccuracy 100, badges{FIRST_PERFECT earned, STREAK_5 prog 0, TEN_QUIZZES prog 0.1}, activityDays 4. `/learner/summary` → {assignedCount 1, avgQuizScore **null**, streakDays 0, continueContent YouTube}.

**🟢 Stat cards + insights reconcile.** Tayinlangan **1** ✓; Tugatilgan **0 · "1 ta biriktirilgandan"** ✓; Umumiy o'zlashtirish **0%** ✓; Ketma-ketlik **0 kun** ✓. Achievements: 🔥 5-kunlik "Ochish uchun 0%" (STREAK_5 prog 0), 🎯 Birinchi mukammal test **"Qo'lga kiritildi"** (FIRST_PERFECT earned), 🏅 10 ta test **"Ochish uchun 10%"** (TEN_QUIZZES 0.1→10%) — all exact. 35-day heatmap + "Shu haftada 1 kun faol" (activityDays incl. 2026-07-14) ✓. Weakest topic "Darsga kirish…" 0% ✓. Topshiriqlar: Written 0/1, Game **1/1 · Oxirgi 100% · 3265 ball** ✓. "Qoldirgan joyingizdan davom eting" → YouTube 0% ✓.

**🟢 Unread-message banner (new feature).** "O'qituvchingizdan 1 ta yangi xabar / QA Owner: Salom! Ertaga matematikadan…" — matches unread-count 1 + newest thread. Bell badge "1". ✓ (Welcome/temp-password banner also shows — legacy per-device onboarding flag, not `mustChangePassword` which is false; known-expected.)

**🟡 O91 (S3, data-consistency — LOGGED low-confidence).** "O'rtacha test" card = **"—"** (`summary.avgQuizScore`=null) while `/learner/progress.avgAccuracy`=**100** for the same learner + a visible 100% GAME result + earned 🎯 "first perfect test" badge. The two quiz-performance figures disagree because `avgQuizScore` counts only written/practice quizzes, not the GAME assessment. Reproducible + two named oracle endpoints, but definitionally ambiguous (is a GAME a "test"?) → morning product review, not filed as F.
...
### C6 — /tenant/students Hostile: XSS-name render + F79 CSV regression + BOM import · Hostile · security/data-integrity (regression bucket, fresh angle)

**Charter:** Re-attack the students page as a hostile actor — verify the stored `<script>` roster name is escaped (not executed), long unbreakable names don't overflow, F79 (CSV formula-injection escaping) still holds, and BOM/encoding imports degrade gracefully. **Done when:** 0 live `<script>` from the payload, no overflow desktop+mobile, the shipped `escapeCsv` neutralizes all formula/RFC-4180 hostile inputs, and a BOM'd import is characterized.

**XSS render is safe (React escaping).** Roster contains test student `Ali <script>alert(1)</script> ОченьДлинное…` (script + very-long single-word Cyrillic + duplicate homoglyphs). Deterministic: **0** `<script>` elements match `alert(1)` (not executed), payload present as **literal escaped text**, docOverflowX=false at 1728px. At 390px: scrollWidth==clientWidth, no overflow (desktop table cell is display:none on mobile; mobile cards clip). No stored-XSS, no layout break from the length extreme.

**F79 CSV formula-injection escaping — regression intact.** Source (`students/page.tsx:142 escapeCsv`) unchanged: formula-guard `/^[=+\-@\t\r]/` prefixes with a single-quote, THEN RFC-4180 quote/double, + UTF-8 BOM prepend. Metamorphic against the SHIPPED regex: `=cmd|calc` to `'=cmd|calc`, `+1+1` to `'+1+1`, `@SUM` to `'@SUM`, `-2+3` to `'-2+3`, tab-lead guarded, **`=1,2,3` to `"'=1,2,3"`** (guard-then-quote order correct), `a"b` to `"a""b"`, unicode survives, benign untouched. Email-less kids' `@username` cell always starts with `@` so it is always quote-prefixed (defensive). F79 holds.

**BOM import — theory disproven LIVE (self-verification win).** Source read suggested a leading U+FEFF would corrupt the first header (`"<BOM>name" != "name"` so idx.name=-1, name dropped). **Live A/B test refuted it:** `POST /students/import` with a `String.fromCharCode(0xFEFF)`-prefixed `name,email,username` CSV created the student with **name "ZZBOM QA Name" intact** (report `created`), identical to the no-BOM control. No name-drop, **no finding** (source hypothesis environment-attributed away by a live repro, per §E). Both test students hard-deleted (roster restored to 5).
...
**7 charters, all completed. 0 new F; 4 new observations (O90 pricing-CTA dead-end, O91 learner avg-quiz cross-source, O92 admin MRR USD-table price drift, O93 429 no Retry-After); F78 (FLAGGED media label-only) re-confirmed LIVE with an evidence bundle. Every commit on `claude/visual-qa`, nothing pushed. No journal↔ledger drift.** This run cleared the two biggest RCRCRC-fresh surfaces — **real-so'm pricing** (`aaaa2b9c`, all 4 tiers × uz/ru, prices reconcile to `PRICING_PLANS` arithmetic) and the **information-dense tutor + student dashboards** (`7ae7893f`, every stat card shadowed back to `/tenant/progress`+`/learner/progress`+`/results` and reconciled) — plus analytics fuzz, moderation re-confirm, students-page security, and the keyboard-a11y criterion.

**Clean passes (oracle-verified, no findings):** C1 pricing (monthly+annual+eff-monthly+20%-save all reconcile; uz+ru i18n clean; CTA routing characterized); C2 tutor dashboard (6 stat cards + class snapshot + assessment x/y all match backend; avg-mastery fix verified self-consistent 0% via per-student mastery not avgCoverage); C3 student dashboard (4 cards + achievements + heatmap + unread-banner reconcile; assigned-only invariant → 404 on non-assigned + redirect); C4 analytics (8ep×11 hostile-days = 88 requests all 200, no NaN/Infinity, empty-DB ÷0 structurally impossible); C5 F78 live re-confirm (flag → learner still served 6 episodes, zero enforcement); C6 students security (stored-XSS React-escaped, no overflow; F79 CSV formula-injection escaping regression intact via metamorphic; BOM-import name-drop theory disproven live); C7 keyboard a11y (Enter-submit, logical tab order, visible focus ring, controls named).

**Coverage advanced (by depth):** 12 cells → `oracle-verified` this run — pricing/logged-out(uz+ru), pricing/INDIVIDUAL, pricing/OWNER, tenant.dashboard/info-dense-stats, learner.dashboard/info-dense-stats, analytics-empty, analytics-populated, analytics.8ep/days-fuzz, xss-longname-render, csv-import-malformed(Hostile), quiz.keyboard-operability; admin.content/flag-effect re-verified live; csv-export/F79 re-verified.

**Invariant sweep (all held):** learner sees only assigned — **404 (not 403, doesn't leak existence) on non-assigned content + redirect** (C3); no cross-tenant id in any response body (C2/C3/C4); INDIVIDUAL never auto-promoted (C1 CTA→register, no state change); seat limit not exceeded (C6 BOM import respected unlimited seatLimit); moderation flag has zero serving-path effect (C5, product gap F78, not an isolation break); GAME timing server-authoritative (untouched this run, R20-verified).

**Flaky-suspect list:** O84 (R19 `/billing/me` one-off 500) — not re-encountered this run; carry forward. No new flaky-suspects.

**Blocked-on-job list:** none stuck. INDIVIDUAL generation quota NOT hit this run (C7 used an existing quiz, no generation). Admin rate-limit bucket briefly exhausted by the C-final 429 burst test (120/60s) — self-recovers within 60s, not a persistent block.

**Staleness report — still `viewed`/∞ or not reached this run:** HOTSPOT + DRAG_DROP learner players (PLANS `QA-DEFER-HOTSPOT-A11Y`, needs owner image-build); **structured-type (MATCHING/ORDERING) keyboard-reorder a11y** (would need ~130s structured-quiz gen; DROPDOWN_CLOZE remains oracle-verified R20); AI **Video** per-part generate/retry+FAILED (unverified, same durationSec estimator as O82); admin content-detail; terms cell; tenant.progress mastery-by-topic/empty; tenant.messages-bell; learner.assessments/malformed-config; content.[id] study-mode/section-rail; ru/en live GAME-player render; O82 podcast duration (PLANS).

**Tomorrow's charter queue (next run):**
1. **Structured-player keyboard a11y (MATCHING/ORDERING)** — generate a structured quiz (accept the ~130s), keyboard-reorder + grading truth-table (the untested a11y half).
2. **tenant.progress + tenant.billing** — mastery-by-topic/empty cells (∞) + the owner billing page price surface (close the price cluster: does /tenant/billing show correct so'm?).
3. **O91 re-triage** — is the learner "O'rtacha test" card meant to include GAME results? Confirm the product intent (promote to F or close). Pairs with O92 (product decision on MRR USD table).
4. **Re-examination bucket:** re-attack the **GAME-live** flow (R20 C3) from a Saboteur angle (network drop mid-round, refresh mid-timer) at ru/en; and **deactivation-mid-session** invariant live (R5 recipe) — not re-tested this run.
5. **admin content-detail + terms** cells (∞, never reached).
