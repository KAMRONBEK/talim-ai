...
- **🟡 F59 (S2) logged, NOT auto-fixed — quiz generation that returns 0 questions / FAILS spins forever.** Two agents independently confirmed: `Quiz` has no `status` column, so a 0-question or failed `generateQuiz` job has nowhere to persist FAILED; `useQuiz` polls until `questions.length` (never, on failure) and `QuizCard`/quiz page render the "generating…" spinner indefinitely with no error/retry. The job already emits an SSE `quiz.status: FAILED` but nothing persists/reads it. The clean fix needs a **schema migration** (`Quiz.status`) — deferred to human review per this session's no-autonomous-migration discipline (the local DB is migration-checksum-drift-prone + the pgvector migrate-diff trap). A frontend-only mitigation (timeout / read the SSE FAILED event) is possible but partial (loses state on reload).

**Why F59 wasn't auto-fixed:** schema migrations on this dev DB are the highest-risk autonomous change (a botched migration could wedge the user's environment with no one present to recover), so — like F11/F45/F46 — it's logged with a precise fix rather than force-applied.
