...
**F71 (S3, admin) — LOGGED (clear but multi-site; deferred to a focused follow-up): admin list pages blank silently + several admin mutations fail silently.** Surfaced by the same code-verification agent; documented with precise pointers for a careful, fully-verified follow-up (avoids sprawling, hard-to-verify interactive edits in an unattended run; the HIGH-severity hangs are already fixed as F70).
  ...
  - **A-3 (content Retry/Delete fire-and-forget):** `content/page.tsx:62,72` call `.mutate()` with no `onError`; `useRetryContent`/`useDeleteContent` (`useAdmin.ts:235-253`) have only `onSuccess` → on failure (404/500) nothing renders, operator thinks it worked.
  - **A-4 (generated delete unhandled):** `generated/page.tsx:22-26` `await api.delete(...)` with no try/catch → unhandled rejection, invalidate never runs, silent.
  - **A-5 (users-list Reset password fire-and-forget):** `users/page.tsx:111` `resetPassword.mutate(...)` no error handling (the Delete next to it IS wrapped) → silent on failure.
  - **A-6 (tenant/sub save silent):** `tenants/[id]/page.tsx:180` `updateTenant.mutate(...)` and `users/[id]/page.tsx:496` `updateSubscription.mutate(...)` have no `isError` UI → "Saving…" flickers back with no confirmation/error.
  - **Recommended fix (per the established admin convention — see `tutor-requests` approve/reject + `users` Delete which DO use `try { await mutateAsync } catch { alert(...) }`):** wrap the bare call sites in try/catch + `alert`, and add an `isError` row to the A-2 lists. NOT a blanket hook-level `onError` — that would double-alert the call sites that already handle errors. | apps/admin pages + useAdmin.ts call sites
