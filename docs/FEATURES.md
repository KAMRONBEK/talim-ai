...
- **Web:** `components/practice/practice-generator.tsx` (the unified dialog; Flashcards is a type chip, no separate mode), `app/[locale]/quiz/[id]/page.tsx`, `components/quiz/*` (all types incl. `FlashcardInput` + partial credit + misconception rationales), `hooks/useQuiz.ts`, `hooks/useMastery.ts`; the standalone spaced-repetition deck UI remains at `app/[locale]/content/[id]/flashcards/page.tsx` (deck reviews feed the same mastery at self-report weight).
