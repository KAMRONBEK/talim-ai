...
- **What:** ONE generator for all practice from a piece of content (design: `docs/plans/question-engine.md`): the learner picks **count** (1–30, default 10), **question types** (multi-select: MCQ, true/false, multi-select, fill-blank, dropdown-cloze, matching, ordering, short answer, numeric, **flashcards** — flashcards are a first-class `FLASHCARD` question type mixed INTO the same session, rendered as reveal-the-back + self-grade "Bildim/Bilmadim", graded via self-report and fed to mastery at half weight), **depth** (recall / understanding / application / mixed; application = near-transfer questions set in NEW scenarios, never parroted from the text), and **scope** (section or whole material). Generation runs a quality pipeline: misconception-derived distractors with per-option rationales, a verbatim `sourceQuote` anchor verified against the material (hallucination firewall), overgenerate → rule-filter → balanced answer-position shuffle, then a **fill-to-count retry pass** (`lib/question-gen.ts`) when the filters leave a shortfall — the re-prompt lists kept stems as forbidden repeats, and per-reason skip counts are logged. Whole-material scope samples an **even spread of chunks** across the document (scaled to count); thin heading-only sections widen to whole-material context. Math is prompted as `$...$` LaTeX and rendered with KaTeX (`components/learning/rich-text.tsx` also normalizes `\(..\)` delimiters and hard-breaks newlines). After submitting, the learner gets a per-question review screen plus **mastery deltas**. Mastery is **Elo-KT** per (user, section): correct answers push it up, wrong answers push it **down**, inactivity decays it; bands (attempted/familiar/proficient/mastered) are gated by evidence so one right answer never shows a topic as "learnt". The legacy "quick check" (QuizKind `QUICK`) is retired (rows relabeled FULL by migration). (Distinct from tenant assessments in §4, which share the same generation + grading engine but exclude FLASHCARD.)
- **Who:** INDIVIDUAL, TENANT_OWNER (create); TENANT_LEARNER (take, via assigned content).
- **API:** `controllers/quiz.controller.ts` (`createQuiz` with `types`/`depth`/`count`, `submitQuiz` returning `masteryDeltas`, `getContentMastery`), job `jobs/generateQuiz.job.ts`; generation engine `lib/question-gen.ts` (fill-to-count loop) + `lib/question-gen-prompt.ts` + `lib/question-postprocess.ts` + `lib/question-builders.ts`; mastery `services/sectionMastery.service.ts` (+ pure math `packages/types/mastery.ts`). Routes: `routes/quiz.routes.ts` (`/quiz/content/:contentId`, `/quiz/content/:contentId/mastery`, `/quiz/:id`, `/quiz/:id/submit`). Models `Quiz` (`depth`, `types`), `QuizQuestion` (`config`, `difficulty`, `bloom`, `sourceQuote`, `optionRationales`, `sourceSectionId`), `QuizAttempt`, `SectionMastery`, `QuestionStat`; usage feature `QUIZ_GEN`. Migrations `20260711000000_question_engine`, `20260712000000_retire_quick_quizzes`, `20260712010000_flashcard_question_type`.
- **Web:** `components/practice/practice-generator.tsx` (the unified dialog; Flashcards is a type chip, no separate mode), `app/[locale]/quiz/[id]/page.tsx`, `components/quiz/*` (all types incl. `FlashcardInput` + partial credit + misconception rationales; `DropdownClozeInput` is an inline cloze — the sentence renders with numbered slot pills in place of the `___` markers and every blank gets its own labeled chip row, judged independently on check), `hooks/useQuiz.ts`, `hooks/useMastery.ts`. The left-panel Resources block has NO flashcards entry — flashcards live only in the Practice dialog; the deck route `app/[locale]/content/[id]/flashcards/page.tsx` stays alive for existing decks/bookmarks (deck reviews feed the same mastery at self-report weight).
