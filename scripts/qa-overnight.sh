...
2) Charter selection (§B): pick 6-10 charters from docs/qa/coverage-map.md by staleness×risk − recentness; recently-tested cells are DEPRIORITIZED, not excluded — a 'green' cell is only proven bug-free under the angles tried so far and this suite is fallible, so RESERVE ≥2 charters each run to re-examine oracle-verified / low-staleness cells from a NEW angle (a different persona × tour × state × input-attack than their tour_last), the only forbidden move being a literal replay of the same persona+tour+steps; write Hendrickson-form charters with pre-committed done-conditions.
