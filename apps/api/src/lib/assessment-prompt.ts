...
/** Styles for the B2B question-bank generator — adds the new B1 types. */
...
function styleInstruction(style: AssessmentQuestionStyle): string {
  switch (style) {
    ...
    case 'mixed':
    default:
      return "Turli xil savollardan foydalaning: ko'p tanlovli (MULTIPLE_CHOICE), to'g'ri/noto'g'ri (TRUE_FALSE), ko'p javobli (MULTIPLE_SELECT), bo'sh joyni to'ldirish (FILL_BLANK), qisqa yozma (SHORT_ANSWER) va, mos bo'lsa, raqamli (NUMERIC). Turlarni muvozanatli aralashtiring.";
  }
}
...
/**
 * B2B assessment normalizer — the full set including the new B1 types
 * (TRUE_FALSE / MULTIPLE_SELECT / FILL_BLANK). Used only by the tenant question-bank
