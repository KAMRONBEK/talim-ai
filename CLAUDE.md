...
Rules:
- **graphify is project memory — consult it FIRST.** Before claiming a feature/fix doesn't exist, or starting git-log/grep archaeology, run `graphify query "<topic>"` / `graphify explain "<concept>"` (and check Claude's recalled auto-memory). They are the source of truth for what the project already has; do not conclude something is missing until you've checked them. (Real miss this prevents: the scanned-PDF OCR already runs **Mistral OCR** via OpenRouter's `file-parser` plugin — `OPENROUTER_OCR_MODEL` is just the throwaway wrapper model.)
