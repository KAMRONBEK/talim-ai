---
source_file: "apps/web/components/learning/summary-text.tsx"
type: "code"
community: "Community 44"
location: "L1"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/Community_44
---

# summary-text.tsx

## Connections
- [[Block]] - `contains` [EXTRACTED]
- [[Section]] - `contains` [EXTRACTED]
- [[SelectionAsk()]] - `imports` [EXTRACTED]
- [[SummaryText()]] - `contains` [EXTRACTED]
- [[asHeading()]] - `contains` [EXTRACTED]
- [[cn()]] - `imports` [EXTRACTED]
- [[content-stage.tsx]] - `imports_from` [EXTRACTED]
- [[countWords()]] - `contains` [EXTRACTED]
- [[format-summary.ts]] - `imports_from` [EXTRACTED]
- [[formatSummaryForDisplay()]] - `imports` [EXTRACTED]
- [[index.ts_3]] - `imports_from` [EXTRACTED]
- [[layout.tsx_4]] - `imports_from` [EXTRACTED]
- [[page.tsx_31]] - `imports_from` [EXTRACTED]
- [[parseSummary()]] - `contains` [EXTRACTED]
- [[selection-ask.tsx]] - `imports_from` [EXTRACTED]
- [[splitLead()]] - `contains` [EXTRACTED]
- [[stripInline()]] - `contains` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/Community_44