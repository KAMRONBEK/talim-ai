---
source_file: "apps/api/package.json"
type: "code"
community: "Community 54"
location: "L5"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/Community_54
---

# scripts

## Connections
- [[build_1]] - `contains` [EXTRACTED]
- [[create-admin]] - `contains` [EXTRACTED]
- [[create-tenant-owner]] - `contains` [EXTRACTED]
- [[dbgenerate]] - `contains` [EXTRACTED]
- [[dbmigrate]] - `contains` [EXTRACTED]
- [[dbpush]] - `contains` [EXTRACTED]
- [[dbseed]] - `contains` [EXTRACTED]
- [[dbstudio]] - `contains` [EXTRACTED]
- [[dev_1]] - `contains` [EXTRACTED]
- [[inspect-chunks]] - `contains` [EXTRACTED]
- [[lint_1]] - `contains` [EXTRACTED]
- [[package.json_1]] - `contains` [EXTRACTED]
- [[prebuild]] - `contains` [EXTRACTED]
- [[predev]] - `contains` [EXTRACTED]
- [[pretypecheck]] - `contains` [EXTRACTED]
- [[start_1]] - `contains` [EXTRACTED]
- [[typecheck_1]] - `contains` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/Community_54