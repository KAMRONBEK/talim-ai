---
source_file: "apps/web/app/[locale]/content/[id]/page.tsx"
type: "code"
community: "Content Workspace & Chat"
location: "L1"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/Content_Workspace__Chat
---

# page.tsx

## Connections
- [[AssignStudentsPanel()]] - `imports` [EXTRACTED]
- [[Button]] - `imports` [EXTRACTED]
- [[ContentDetailPage()]] - `contains` [EXTRACTED]
- [[ContentLearnPanel()]] - `imports` [EXTRACTED]
- [[ContentLearnPanelSheet()]] - `imports` [EXTRACTED]
- [[ContentPageLoading()]] - `contains` [EXTRACTED]
- [[ContentStage()]] - `imports` [EXTRACTED]
- [[ContentStatusGate()]] - `imports` [EXTRACTED]
- [[ContentWorkspaceInner()]] - `contains` [EXTRACTED]
- [[DeleteContentDialog()]] - `imports` [EXTRACTED]
- [[Dialog()]] - `imports` [EXTRACTED]
- [[DialogContent()]] - `imports` [EXTRACTED]
- [[DialogHeader()]] - `imports` [EXTRACTED]
- [[DialogTitle()]] - `imports` [EXTRACTED]
- [[LearnTab]] - `imports` [EXTRACTED]
- [[ResizableSplit()]] - `imports` [EXTRACTED]
- [[StageExcerpt]] - `imports` [EXTRACTED]
- [[SummaryText()]] - `imports` [EXTRACTED]
- [[assign-students-panel.tsx]] - `imports_from` [EXTRACTED]
- [[auth-routing.ts]] - `imports_from` [EXTRACTED]
- [[content-learn-panel.tsx]] - `imports_from` [EXTRACTED]
- [[content-stage.tsx]] - `imports_from` [EXTRACTED]
- [[content-status-gate.tsx]] - `imports_from` [EXTRACTED]
- [[delete-content-dialog.tsx]] - `imports_from` [EXTRACTED]
- [[getHomePathForRole()]] - `imports` [EXTRACTED]
- [[index.ts_3]] - `imports_from` [EXTRACTED]
- [[navigation.ts]] - `imports_from` [EXTRACTED]
- [[resizable-split.tsx]] - `imports_from` [EXTRACTED]
- [[summary-text.tsx]] - `imports_from` [EXTRACTED]
- [[useChatStore]] - `imports` [EXTRACTED]
- [[useChatStore.ts]] - `imports_from` [EXTRACTED]
- [[useContent()]] - `imports` [EXTRACTED]
- [[useContent.ts]] - `imports_from` [EXTRACTED]
- [[useContentActions()]] - `imports` [EXTRACTED]
- [[useContentActions.ts]] - `imports_from` [EXTRACTED]
- [[useContentProgress()]] - `imports` [EXTRACTED]
- [[useLearningHistory()]] - `imports` [EXTRACTED]
- [[useLimitErrorHandler()]] - `imports` [EXTRACTED]
- [[useLimitErrorHandler.ts]] - `imports_from` [EXTRACTED]
- [[useProgress.ts]] - `imports_from` [EXTRACTED]
- [[useReparseContent()]] - `imports` [EXTRACTED]
- [[useReparseContent.ts]] - `imports_from` [EXTRACTED]
- [[useSection()]] - `imports` [EXTRACTED]
- [[useSections()]] - `imports` [EXTRACTED]
- [[useSections.ts]] - `imports_from` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/Content_Workspace__Chat