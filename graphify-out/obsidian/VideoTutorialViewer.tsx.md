---
source_file: "apps/web/components/learning/VideoTutorialViewer.tsx"
type: "code"
community: "Transcript Panel"
location: "L1"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/Transcript_Panel
---

# VideoTutorialViewer.tsx

## Connections
- [[TranscriptExcerptPayload]] - `imports` [EXTRACTED]
- [[TranscriptPanel()]] - `imports` [EXTRACTED]
- [[TranscriptPanel.tsx]] - `imports_from` [EXTRACTED]
- [[TranscriptSegment]] - `imports` [EXTRACTED]
- [[VideoChapter]] - `contains` [EXTRACTED]
- [[VideoTutorialViewer()]] - `contains` [EXTRACTED]
- [[VideoTutorialViewerProps]] - `contains` [EXTRACTED]
- [[YoutubeVideoPlayer()]] - `imports` [EXTRACTED]
- [[YoutubeVideoPlayer.tsx]] - `imports_from` [EXTRACTED]
- [[cn()]] - `imports` [EXTRACTED]
- [[content-stage.tsx]] - `imports_from` [EXTRACTED]
- [[deriveChapters()]] - `contains` [EXTRACTED]
- [[formatChapterTime()]] - `contains` [EXTRACTED]
- [[formatTranscriptExcerpt()]] - `imports` [EXTRACTED]
- [[getYoutubeVideoId()]] - `imports` [EXTRACTED]
- [[index.ts_2]] - `imports_from` [EXTRACTED]
- [[index.ts_3]] - `imports_from` [EXTRACTED]
- [[useTranscript()]] - `imports` [EXTRACTED]
- [[useTranscript.ts]] - `imports_from` [EXTRACTED]
- [[youtube.ts]] - `imports_from` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/Transcript_Panel