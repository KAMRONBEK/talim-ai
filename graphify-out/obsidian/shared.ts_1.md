---
source_file: "apps/api/src/services/assessment/shared.ts"
type: "code"
community: "Assessments Service"
location: "L1"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/Assessments_Service
---

# shared.ts

## Connections
- [[AppError]] - `imports` [EXTRACTED]
- [[GeneratedQuestion]] - `contains` [EXTRACTED]
- [[LeaderboardAttempt]] - `contains` [EXTRACTED]
- [[QuestionStyle_1]] - `contains` [EXTRACTED]
- [[SubmitAnswerValue]] - `contains` [EXTRACTED]
- [[assertBank()]] - `contains` [EXTRACTED]
- [[assertLearnerAssignment()]] - `contains` [EXTRACTED]
- [[assertTenantContentIds()]] - `contains` [EXTRACTED]
- [[assessments.ts]] - `imports_from` [EXTRACTED]
- [[assignAssessmentSchema]] - `contains` [EXTRACTED]
- [[banks.ts]] - `imports_from` [EXTRACTED]
- [[buildRagContext()]] - `imports` [EXTRACTED]
- [[chunk-sampling.ts]] - `imports_from` [EXTRACTED]
- [[computeGamePoints()]] - `contains` [EXTRACTED]
- [[createAssessmentSchema]] - `contains` [EXTRACTED]
- [[createBankSchema]] - `contains` [EXTRACTED]
- [[createQuestionSchema]] - `contains` [EXTRACTED]
- [[error.middleware.ts]] - `imports_from` [EXTRACTED]
- [[formatAssessment()]] - `contains` [EXTRACTED]
- [[formatBank()]] - `contains` [EXTRACTED]
- [[formatQuestion()]] - `contains` [EXTRACTED]
- [[generateSchema]] - `contains` [EXTRACTED]
- [[getSectionContext()]] - `contains` [EXTRACTED]
- [[goLiveAssessmentSchema]] - `contains` [EXTRACTED]
- [[index.ts_2]] - `re_exports` [EXTRACTED]
- [[jsonStringArray()]] - `imports` [EXTRACTED]
- [[learner.ts]] - `imports_from` [EXTRACTED]
- [[learnerDisplayName()]] - `contains` [EXTRACTED]
- [[parseQuestionConfig()]] - `imports` [EXTRACTED]
- [[patchQuestionSchema]] - `contains` [EXTRACTED]
- [[prisma_2]] - `imports` [EXTRACTED]
- [[prisma.ts]] - `imports_from` [EXTRACTED]
- [[question-builders.ts]] - `imports_from` [EXTRACTED]
- [[question-gen.ts]] - `imports_from` [EXTRACTED]
- [[question-postprocess.ts]] - `imports_from` [EXTRACTED]
- [[questionDepthEnum]] - `contains` [EXTRACTED]
- [[questionStyleEnum]] - `contains` [EXTRACTED]
- [[questionTypeEnum]] - `contains` [EXTRACTED]
- [[quiz.controller.ts]] - `imports_from` [EXTRACTED]
- [[rag.service.ts]] - `imports_from` [EXTRACTED]
- [[results.ts]] - `imports_from` [EXTRACTED]
- [[sampleChunksEvenly()]] - `imports` [EXTRACTED]
- [[scheduleAssessmentSchema]] - `contains` [EXTRACTED]
- [[submitAnswerValueSchema]] - `contains` [EXTRACTED]
- [[submitAssessmentSchema]] - `contains` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/Assessments_Service