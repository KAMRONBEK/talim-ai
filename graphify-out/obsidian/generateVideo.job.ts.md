---
source_file: "apps/api/src/jobs/generateVideo.job.ts"
type: "code"
community: "Background Jobs & Queues"
location: "L1"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/Background_Jobs__Queues
---

# generateVideo.job.ts

## Connections
- [[AppLocale]] - `imports` [EXTRACTED]
- [[Deck]] - `imports` [EXTRACTED]
- [[GenerateVideoJobData]] - `imports` [EXTRACTED]
- [[LANGUAGE_NAME]] - `contains` [EXTRACTED]
- [[StorageService]] - `imports` [EXTRACTED]
- [[StoredSegment_1]] - `contains` [EXTRACTED]
- [[ai.service.ts]] - `imports_from` [EXTRACTED]
- [[buildNarrations()]] - `contains` [EXTRACTED]
- [[estimateDurationSec()]] - `contains` [EXTRACTED]
- [[generateAndStoreSlideDeck()]] - `imports` [EXTRACTED]
- [[generateJsonCompletion()]] - `imports` [EXTRACTED]
- [[getSlideDeck()]] - `imports` [EXTRACTED]
- [[index.ts]] - `imports_from` [EXTRACTED]
- [[index.ts_2]] - `imports_from` [EXTRACTED]
- [[jobEvents]] - `imports` [EXTRACTED]
- [[jobEvents.service.ts]] - `imports_from` [EXTRACTED]
- [[parseAppLocale()]] - `imports` [EXTRACTED]
- [[prisma_2]] - `imports` [EXTRACTED]
- [[prisma.ts]] - `imports_from` [EXTRACTED]
- [[queue.service.ts]] - `imports_from` [EXTRACTED]
- [[registerGenerateVideoJob()]] - `contains` [EXTRACTED]
- [[slideToText()]] - `contains` [EXTRACTED]
- [[slides.service.ts]] - `imports_from` [EXTRACTED]
- [[storage.service.ts]] - `imports_from` [EXTRACTED]
- [[synthesizeSpeech()]] - `imports` [EXTRACTED]
- [[tts.service.ts]] - `imports_from` [EXTRACTED]
- [[videoQueue]] - `imports` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/Background_Jobs__Queues