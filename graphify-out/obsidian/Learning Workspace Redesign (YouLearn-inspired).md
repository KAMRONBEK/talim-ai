---
source_file: "docs/plans/youlearn-redesign.md"
type: "document"
community: "Isolation Guard & Visual Direction"
tags:
  - graphify/document
  - graphify/EXTRACTED
  - community/Isolation_Guard__Visual_Direction
---

# Learning Workspace Redesign (YouLearn-inspired)

## Connections
- [[Citations (Manba Pills)]] - `references` [EXTRACTED]
- [[Interactive Quiz Learn Player]] - `references` [EXTRACTED]
- [[Learn Hub Panel (ContentLearnPanel)]] - `references` [EXTRACTED]
- [[Persistent Study Workspace (3-pane)]] - `references` [EXTRACTED]
- [[Role Adaptations (server-side security)]] - `references` [EXTRACTED]
- [[Visual Direction (girih, marker-highlight)]] - `references` [EXTRACTED]

#graphify/document #graphify/EXTRACTED #community/Isolation_Guard__Visual_Direction