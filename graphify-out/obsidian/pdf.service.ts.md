---
source_file: "apps/api/src/services/pdf.service.ts"
type: "code"
community: "src services · usage-pricing.ts"
location: "L1"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/src_services__usage-pricingts
---

# pdf.service.ts

## Connections
- [[Env]] - `imports` [EXTRACTED]
- [[OpenRouterFileAnnotation]] - `contains` [EXTRACTED]
- [[UsageContext]] - `imports` [EXTRACTED]
- [[content.controller.ts_1]] - `imports_from` [EXTRACTED]
- [[env.ts]] - `imports_from` [EXTRACTED]
- [[extractPdfPageRange()]] - `contains` [EXTRACTED]
- [[extractPdfText()]] - `contains` [EXTRACTED]
- [[extractRegionTextFromImage()]] - `contains` [EXTRACTED]
- [[extractTextFromPageImages()]] - `contains` [EXTRACTED]
- [[extractWithOpenAI()]] - `contains` [EXTRACTED]
- [[extractWithPdfParse()]] - `contains` [EXTRACTED]
- [[getPdfPageCount()]] - `contains` [EXTRACTED]
- [[hasPrimaryOcrProvider()]] - `contains` [EXTRACTED]
- [[ocrImageDataUrl()]] - `contains` [EXTRACTED]
- [[ocrRequestOnce()]] - `contains` [EXTRACTED]
- [[ocrViaOpenRouter()]] - `contains` [EXTRACTED]
- [[processContent.job.ts]] - `imports_from` [EXTRACTED]
- [[rasterizeAndOcrPdf()]] - `contains` [EXTRACTED]
- [[recordUsage()]] - `imports` [EXTRACTED]
- [[runCli()]] - `contains` [EXTRACTED]
- [[runPdftoppm()]] - `contains` [EXTRACTED]
- [[tenant-content.controller.ts]] - `imports_from` [EXTRACTED]
- [[usage.service.ts]] - `imports_from` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/src_services__usage-pricingts