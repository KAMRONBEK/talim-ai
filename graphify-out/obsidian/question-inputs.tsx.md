---
source_file: "apps/web/components/quiz/question-inputs.tsx"
type: "code"
community: "Quiz Player UI"
location: "L1"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/Quiz_Player_UI
---

# question-inputs.tsx

## Connections
- [[DropdownClozeInput()]] - `contains` [EXTRACTED]
- [[FillBlankInput()]] - `contains` [EXTRACTED]
- [[FlashcardInput()]] - `contains` [EXTRACTED]
- [[Input]] - `imports` [EXTRACTED]
- [[MatchingInput()]] - `contains` [EXTRACTED]
- [[MultipleSelectInput()]] - `contains` [EXTRACTED]
- [[OrderingInput()]] - `contains` [EXTRACTED]
- [[QuestionType]] - `imports` [EXTRACTED]
- [[QuizAnswerValue]] - `contains` [EXTRACTED]
- [[QuizCard.tsx]] - `imports_from` [EXTRACTED]
- [[QuizQuestion]] - `imports` [EXTRACTED]
- [[RichText()]] - `imports` [EXTRACTED]
- [[TYPE_LABEL_KEYS]] - `contains` [EXTRACTED]
- [[TrueFalseInput()]] - `contains` [EXTRACTED]
- [[acceptedAnswers()]] - `contains` [EXTRACTED]
- [[acceptedForBlank()]] - `contains` [EXTRACTED]
- [[blankCount()]] - `contains` [EXTRACTED]
- [[clozeOptions()_1]] - `contains` [EXTRACTED]
- [[formatAnswerDisplay()]] - `contains` [EXTRACTED]
- [[gradableQuestion()]] - `contains` [EXTRACTED]
- [[index.ts_2]] - `imports_from` [EXTRACTED]
- [[index.ts_3]] - `imports_from` [EXTRACTED]
- [[isAiCheckable()]] - `contains` [EXTRACTED]
- [[isAiJudgedQuestionType()]] - `imports` [EXTRACTED]
- [[isAnswerProvided()]] - `contains` [EXTRACTED]
- [[jsonStringArray()]] - `imports` [EXTRACTED]
- [[matchesAcceptedAnswer()]] - `imports` [EXTRACTED]
- [[matchingLeft()_1]] - `contains` [EXTRACTED]
- [[normalizeAnswer()]] - `imports` [EXTRACTED]
- [[optionRevealClass()]] - `contains` [EXTRACTED]
- [[page.tsx_40]] - `imports_from` [EXTRACTED]
- [[parseQuestionConfig()]] - `imports` [EXTRACTED]
- [[partialCreditUnits()]] - `contains` [EXTRACTED]
- [[questionRenderKind]] - `contains` [EXTRACTED]
- [[questionTypeLabelKey()]] - `contains` [EXTRACTED]
- [[resolveAcceptedAnswers()]] - `imports` [EXTRACTED]
- [[revealBorder()]] - `contains` [EXTRACTED]
- [[rich-text.tsx]] - `imports_from` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/Quiz_Player_UI