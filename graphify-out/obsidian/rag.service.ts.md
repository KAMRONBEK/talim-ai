---
source_file: "apps/api/src/services/rag.service.ts"
type: "code"
community: "Community 32"
location: "L1"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/Community_32
---

# rag.service.ts

## Connections
- [[AppLocale]] - `imports` [EXTRACTED]
- [[Env]] - `imports` [EXTRACTED]
- [[FIGURE_LABEL]] - `contains` [EXTRACTED]
- [[UsageContext]] - `imports` [EXTRACTED]
- [[boundContextByTokens()]] - `contains` [EXTRACTED]
- [[buildFigureContext()]] - `contains` [EXTRACTED]
- [[buildRagContext()]] - `contains` [EXTRACTED]
- [[chat.controller.ts]] - `imports_from` [EXTRACTED]
- [[chunkText()]] - `contains` [EXTRACTED]
- [[countTokens()]] - `contains` [EXTRACTED]
- [[embed.service.ts]] - `imports_from` [EXTRACTED]
- [[embeddingToSql()]] - `imports` [EXTRACTED]
- [[env.ts]] - `imports_from` [EXTRACTED]
- [[generateEmbedding()]] - `imports` [EXTRACTED]
- [[generateEmbeddings()]] - `imports` [EXTRACTED]
- [[generatePodcast.job.ts]] - `imports_from` [EXTRACTED]
- [[generateQuiz.job.ts]] - `imports_from` [EXTRACTED]
- [[getOrderedChunks()]] - `contains` [EXTRACTED]
- [[getRagChunkLabel()]] - `imports` [EXTRACTED]
- [[index.ts_2]] - `imports_from` [EXTRACTED]
- [[ingest.service.ts]] - `imports_from` [EXTRACTED]
- [[inspect-chunks.ts]] - `imports_from` [EXTRACTED]
- [[locale-prompts.ts]] - `imports_from` [EXTRACTED]
- [[mergeSimilarChunks()]] - `contains` [EXTRACTED]
- [[prisma_2]] - `imports` [EXTRACTED]
- [[prisma.ts]] - `imports_from` [EXTRACTED]
- [[processContent.job.ts]] - `imports_from` [EXTRACTED]
- [[rerank()]] - `contains` [EXTRACTED]
- [[scriptVariants()]] - `imports` [EXTRACTED]
- [[searchSimilarChunks()]] - `contains` [EXTRACTED]
- [[searchSimilarFigures()]] - `contains` [EXTRACTED]
- [[shared.ts_1]] - `imports_from` [EXTRACTED]
- [[slides.service.ts]] - `imports_from` [EXTRACTED]
- [[splitByTokens()]] - `contains` [EXTRACTED]
- [[storeChunksWithEmbeddings()]] - `contains` [EXTRACTED]
- [[summary.controller.ts]] - `imports_from` [EXTRACTED]
- [[toBlocks()]] - `contains` [EXTRACTED]
- [[tokenTail()]] - `contains` [EXTRACTED]
- [[usage.service.ts]] - `imports_from` [EXTRACTED]
- [[uzbek-translit.ts]] - `imports_from` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/Community_32