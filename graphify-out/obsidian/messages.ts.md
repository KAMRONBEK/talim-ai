---
source_file: "apps/api/src/services/tenant/messages.ts"
type: "code"
community: "Community 73"
location: "L1"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/Community_73
---

# messages.ts

## Connections
- [[AppError]] - `imports` [EXTRACTED]
- [[SentMessageShape]] - `contains` [EXTRACTED]
- [[error.middleware.ts]] - `imports_from` [EXTRACTED]
- [[formatSentMessage()]] - `contains` [EXTRACTED]
- [[getLearnerUnreadCount()]] - `contains` [EXTRACTED]
- [[getOwnerUnreadReplyCount()]] - `contains` [EXTRACTED]
- [[listLearnerMessages()]] - `contains` [EXTRACTED]
- [[listTenantMessageThreads()]] - `contains` [EXTRACTED]
- [[markLearnerMessageRead()]] - `contains` [EXTRACTED]
- [[markOwnerReplyRead()]] - `contains` [EXTRACTED]
- [[markRecipientRead()]] - `contains` [EXTRACTED]
- [[prisma_2]] - `imports` [EXTRACTED]
- [[prisma.ts]] - `imports_from` [EXTRACTED]
- [[replyMessageSchema]] - `contains` [EXTRACTED]
- [[replyToTenantMessage()]] - `contains` [EXTRACTED]
- [[resolveSenderNames()]] - `contains` [EXTRACTED]
- [[respondToStudentReply()]] - `contains` [EXTRACTED]
- [[sendMessageSchema]] - `contains` [EXTRACTED]
- [[sendTenantMessage()]] - `contains` [EXTRACTED]
- [[tenant.service.ts]] - `re_exports` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/Community_73