---
source_file: "apps/api/src/services/youtube.service.ts"
type: "code"
community: "Community 55"
location: "L1"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/Community_55
---

# youtube.service.ts

## Connections
- [[Env]] - `imports` [EXTRACTED]
- [[OpenAITranscriptionSegment]] - `contains` [EXTRACTED]
- [[OpenAIVerboseTranscription]] - `contains` [EXTRACTED]
- [[TranscriptSegmentInput]] - `contains` [EXTRACTED]
- [[TranscriptSegmentSource]] - `contains` [EXTRACTED]
- [[UsageContext]] - `imports` [EXTRACTED]
- [[YoutubeTranscriptItem]] - `contains` [EXTRACTED]
- [[YoutubeTranscriptResult]] - `contains` [EXTRACTED]
- [[buildTranscriptionPrompt()]] - `contains` [EXTRACTED]
- [[cleanTranscriptText()]] - `contains` [EXTRACTED]
- [[content-shared.ts]] - `imports_from` [EXTRACTED]
- [[content.controller.ts_1]] - `imports_from` [EXTRACTED]
- [[env.ts]] - `imports_from` [EXTRACTED]
- [[extractYoutubeAudio()]] - `contains` [EXTRACTED]
- [[extractYoutubeTranscript()]] - `contains` [EXTRACTED]
- [[extractYoutubeVideoId()]] - `contains` [EXTRACTED]
- [[fallbackTextSegments()]] - `contains` [EXTRACTED]
- [[generateYoutubeTranscript()]] - `contains` [EXTRACTED]
- [[normalizeCaptionSegments()]] - `contains` [EXTRACTED]
- [[normalizeTranscriptionSegments()]] - `contains` [EXTRACTED]
- [[processContent.job.ts]] - `imports_from` [EXTRACTED]
- [[recordUsage()]] - `imports` [EXTRACTED]
- [[streamToBuffer()]] - `contains` [EXTRACTED]
- [[tenant-content.controller.ts]] - `imports_from` [EXTRACTED]
- [[usage.service.ts]] - `imports_from` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/Community_55