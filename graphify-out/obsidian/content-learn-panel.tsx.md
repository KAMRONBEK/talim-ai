---
source_file: "apps/web/components/layout/content-learn-panel.tsx"
type: "code"
community: "Content Workspace & Chat"
location: "L1"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/Content_Workspace__Chat
---

# content-learn-panel.tsx

## Connections
- [[ChatWindow()]] - `imports` [EXTRACTED]
- [[ChatWindow.tsx]] - `imports_from` [EXTRACTED]
- [[ContentLearnPanel()]] - `contains` [EXTRACTED]
- [[ContentLearnPanelBody()]] - `contains` [EXTRACTED]
- [[ContentLearnPanelProps]] - `contains` [EXTRACTED]
- [[ContentLearnPanelSheet()]] - `contains` [EXTRACTED]
- [[ContentLearnPanelSheetProps]] - `contains` [EXTRACTED]
- [[ContentRightPanelBody()]] - `imports` [EXTRACTED]
- [[ContentRightPanelBodyProps]] - `imports` [EXTRACTED]
- [[LearnTab]] - `contains` [EXTRACTED]
- [[Sheet()]] - `imports` [EXTRACTED]
- [[SheetContent()]] - `imports` [EXTRACTED]
- [[SheetHeader()]] - `imports` [EXTRACTED]
- [[SheetTitle()]] - `imports` [EXTRACTED]
- [[cn()]] - `imports` [EXTRACTED]
- [[content-right-panel.tsx]] - `imports_from` [EXTRACTED]
- [[index.ts_3]] - `imports_from` [EXTRACTED]
- [[page.tsx_31]] - `imports_from` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/Content_Workspace__Chat