---
source_file: "apps/api/package.json"
type: "code"
community: "API Dev Tooling"
location: "L42"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/API_Dev_Tooling
---

# devDependencies

## Connections
- [[@talimeslint-config_1]] - `contains` [EXTRACTED]
- [[@talimtsconfig_1]] - `contains` [EXTRACTED]
- [[@typesbcrypt]] - `contains` [EXTRACTED]
- [[@typesbull]] - `contains` [EXTRACTED]
- [[@typescors]] - `contains` [EXTRACTED]
- [[@typesexpress]] - `contains` [EXTRACTED]
- [[@typesjsonwebtoken]] - `contains` [EXTRACTED]
- [[@typesmulter]] - `contains` [EXTRACTED]
- [[@typesnode_1]] - `contains` [EXTRACTED]
- [[@typespdf-parse]] - `contains` [EXTRACTED]
- [[eslint_1]] - `contains` [EXTRACTED]
- [[package.json_1]] - `contains` [EXTRACTED]
- [[prisma]] - `contains` [EXTRACTED]
- [[tsx]] - `contains` [EXTRACTED]
- [[typescript_1]] - `contains` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/API_Dev_Tooling