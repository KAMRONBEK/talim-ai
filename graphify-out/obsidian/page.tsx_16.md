---
source_file: "apps/web/app/[locale]/(learner)/learner/dashboard/page.tsx"
type: "code"
community: "Content Grid & Learner Dashboard"
location: "L1"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/Content_Grid__Learner_Dashboard
---

# page.tsx

## Connections
- [[AppLocale]] - `imports` [EXTRACTED]
- [[AssignedMaterialCard()]] - `contains` [EXTRACTED]
- [[Button]] - `imports` [EXTRACTED]
- [[CardStatus]] - `contains` [EXTRACTED]
- [[Content]] - `imports` [EXTRACTED]
- [[ContentType]] - `imports` [EXTRACTED]
- [[LearnerDashboardPage()]] - `contains` [EXTRACTED]
- [[LearnerMaterial]] - `imports` [EXTRACTED]
- [[Progress()]] - `imports` [EXTRACTED]
- [[StudentWelcomeBanner()]] - `imports` [EXTRACTED]
- [[cn()]] - `imports` [EXTRACTED]
- [[format-relative-time.ts]] - `imports_from` [EXTRACTED]
- [[formatRelativeTime()]] - `imports` [EXTRACTED]
- [[index.ts_2]] - `imports_from` [EXTRACTED]
- [[index.ts_3]] - `imports_from` [EXTRACTED]
- [[navigation.ts]] - `imports_from` [EXTRACTED]
- [[student-welcome-banner.tsx]] - `imports_from` [EXTRACTED]
- [[typeStyles]] - `contains` [EXTRACTED]
- [[useAssessments.ts]] - `imports_from` [EXTRACTED]
- [[useContent.ts]] - `imports_from` [EXTRACTED]
- [[useContents()]] - `imports` [EXTRACTED]
- [[useLearnerAssessments()]] - `imports` [EXTRACTED]
- [[useLearnerMaterials()]] - `imports` [EXTRACTED]
- [[useLearnerSummary()]] - `imports` [EXTRACTED]
- [[useTenant.ts]] - `imports_from` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/Content_Grid__Learner_Dashboard