---
source_file: "apps/web/components/marketing/landing-page.tsx"
type: "code"
community: "Marketing Landing UI"
location: "L1"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/Marketing_Landing_UI
---

# landing-page.tsx

## Connections
- [[Cta()]] - `imports` [EXTRACTED]
- [[FEATURES]] - `imports` [EXTRACTED]
- [[Faq()]] - `imports` [EXTRACTED]
- [[Footer()]] - `imports` [EXTRACTED]
- [[ForTutors()]] - `imports` [EXTRACTED]
- [[Hero()]] - `imports` [EXTRACTED]
- [[HowItWorks()]] - `imports` [EXTRACTED]
- [[LandingPage()]] - `contains` [EXTRACTED]
- [[Navbar()]] - `imports` [EXTRACTED]
- [[PricingSection()]] - `imports` [EXTRACTED]
- [[Stats()]] - `imports` [EXTRACTED]
- [[Testimonials()]] - `imports` [EXTRACTED]
- [[cta.tsx]] - `imports_from` [EXTRACTED]
- [[faq.tsx]] - `imports_from` [EXTRACTED]
- [[features.tsx]] - `imports_from` [EXTRACTED]
- [[footer.tsx]] - `imports_from` [EXTRACTED]
- [[for-tutors.tsx]] - `imports_from` [EXTRACTED]
- [[hero.tsx]] - `imports_from` [EXTRACTED]
- [[how-it-works.tsx]] - `imports_from` [EXTRACTED]
- [[navbar.tsx]] - `imports_from` [EXTRACTED]
- [[page.tsx_38]] - `imports_from` [EXTRACTED]
- [[pricing-section.tsx]] - `imports_from` [EXTRACTED]
- [[stats.tsx]] - `imports_from` [EXTRACTED]
- [[testimonials.tsx]] - `imports_from` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/Marketing_Landing_UI