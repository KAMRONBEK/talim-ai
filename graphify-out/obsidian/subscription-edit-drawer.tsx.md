---
source_file: "apps/admin/components/subscription-edit-drawer.tsx"
type: "code"
community: "Subscription Edit Drawer"
location: "L1"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/Subscription_Edit_Drawer
---

# subscription-edit-drawer.tsx

## Connections
- [[AdminSubscriptionListItem]] - `imports` [EXTRACTED]
- [[Button]] - `imports` [EXTRACTED]
- [[DrawerBody()]] - `contains` [EXTRACTED]
- [[DrawerBodyProps]] - `contains` [EXTRACTED]
- [[FieldLabel()]] - `contains` [EXTRACTED]
- [[PlanCode]] - `imports` [EXTRACTED]
- [[STATUS_LABELS]] - `contains` [EXTRACTED]
- [[STATUS_OPTIONS_3]] - `contains` [EXTRACTED]
- [[Sheet()]] - `imports` [EXTRACTED]
- [[SheetContent()]] - `imports` [EXTRACTED]
- [[SubscriptionEditDrawer()]] - `contains` [EXTRACTED]
- [[SubscriptionEditDrawerProps]] - `contains` [EXTRACTED]
- [[SubscriptionStatus]] - `imports` [EXTRACTED]
- [[TENANT_PLANS_1]] - `contains` [EXTRACTED]
- [[USER_PLANS]] - `contains` [EXTRACTED]
- [[formatDate()]] - `contains` [EXTRACTED]
- [[index.ts_2]] - `imports_from` [EXTRACTED]
- [[index.ts_3]] - `imports_from` [EXTRACTED]
- [[initials()]] - `contains` [EXTRACTED]
- [[page.tsx_4]] - `imports_from` [EXTRACTED]
- [[planLabel()]] - `imports` [EXTRACTED]
- [[useAdmin.ts]] - `imports_from` [EXTRACTED]
- [[useUpdateTenant()]] - `imports` [EXTRACTED]
- [[useUpdateUserSubscription()]] - `imports` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/Subscription_Edit_Drawer