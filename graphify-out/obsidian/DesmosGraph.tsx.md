---
source_file: "apps/web/components/chat/DesmosGraph.tsx"
type: "code"
community: "components chat · DesmosGraph.tsx"
location: "L1"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/components_chat__DesmosGraphtsx
---

# DesmosGraph.tsx

## Connections
- [[DesmosCalculator]] - `contains` [EXTRACTED]
- [[DesmosGraph()]] - `contains` [EXTRACTED]
- [[DesmosGraphPayload]] - `imports` [EXTRACTED]
- [[FallbackGraph]] - `contains` [EXTRACTED]
- [[FallbackPoint]] - `contains` [EXTRACTED]
- [[FallbackSvgGraph()]] - `contains` [EXTRACTED]
- [[VisualBlockRenderer.tsx]] - `imports_from` [EXTRACTED]
- [[Window]] - `contains` [EXTRACTED]
- [[axisLine()]] - `contains` [EXTRACTED]
- [[buildFallbackGraph()]] - `contains` [EXTRACTED]
- [[evaluateFallbackExpression()]] - `contains` [EXTRACTED]
- [[fallbackPath()]] - `contains` [EXTRACTED]
- [[index.ts_2]] - `imports_from` [EXTRACTED]
- [[loadDesmosScript()]] - `contains` [EXTRACTED]
- [[toFallbackExpression()]] - `contains` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/components_chat__DesmosGraphtsx