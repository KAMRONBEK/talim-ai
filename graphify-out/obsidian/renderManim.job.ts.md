---
source_file: "apps/api/src/jobs/renderManim.job.ts"
type: "code"
community: "Env Config & Jobs"
location: "L1"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/Env_Config__Jobs
---

# renderManim.job.ts

## Connections
- [[Env]] - `imports` [EXTRACTED]
- [[ManimPayload]] - `imports` [EXTRACTED]
- [[RenderManimJobData]] - `imports` [EXTRACTED]
- [[StorageService]] - `imports` [EXTRACTED]
- [[VisualBlock]] - `imports` [EXTRACTED]
- [[buildFailedBlock()]] - `contains` [EXTRACTED]
- [[buildReadyBlock()]] - `contains` [EXTRACTED]
- [[chat.controller.ts]] - `imports_from` [EXTRACTED]
- [[env.ts]] - `imports_from` [EXTRACTED]
- [[execFileAsync]] - `contains` [EXTRACTED]
- [[index.ts]] - `imports_from` [EXTRACTED]
- [[index.ts_2]] - `imports_from` [EXTRACTED]
- [[manimQueue]] - `imports` [EXTRACTED]
- [[prisma_2]] - `imports` [EXTRACTED]
- [[prisma.ts]] - `imports_from` [EXTRACTED]
- [[queue.service.ts]] - `imports_from` [EXTRACTED]
- [[registerRenderManimJob()]] - `contains` [EXTRACTED]
- [[renderFallbackSvg()]] - `contains` [EXTRACTED]
- [[renderWithManimCli()]] - `contains` [EXTRACTED]
- [[replaceManimBlockInText()]] - `contains` [EXTRACTED]
- [[resolveManimAsset()]] - `contains` [EXTRACTED]
- [[storage.service.ts]] - `imports_from` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/Env_Config__Jobs