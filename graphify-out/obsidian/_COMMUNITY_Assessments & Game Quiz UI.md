---
type: community
cohesion: 0.05
members: 72
---

# Assessments & Game Quiz UI

**Cohesion:** 0.05 - loosely connected
**Members:** 72 nodes

## Members
- [[ANSWER_CHIPS]] - code - apps/web/components/learner/game-quiz-player.tsx
- [[AssessmentAssignment]] - code - packages/types/index.ts
- [[AssessmentLeaderboard]] - code - packages/types/index.ts
- [[AssessmentLeaderboardRow]] - code - packages/types/index.ts
- [[AssessmentMode]] - code - packages/types/index.ts
- [[AssessmentResults]] - code - packages/types/index.ts
- [[AssessmentSubmitResult]] - code - packages/types/index.ts
- [[BLOOM_LABEL_KEYS]] - code - apps/web/app/[locale]/(tenant)/tenant/assessments/page.tsx
- [[BankQuestion]] - code - packages/types/index.ts
- [[BankQuestionEditableFields]] - code - apps/web/hooks/useAssessments.ts
- [[BankQuestionStatus]] - code - packages/types/index.ts
- [[BankQuestionStyle]] - code - apps/web/hooks/useAssessments.ts
- [[CHOICE_TYPES]] - code - apps/web/components/tenant/question-editor.tsx
- [[DEPTH_HINT_KEYS]] - code - apps/web/app/[locale]/(tenant)/tenant/assessments/page.tsx
- [[DEPTH_OPTIONS]] - code - apps/web/app/[locale]/(tenant)/tenant/assessments/page.tsx
- [[DIFFICULTY_BADGES]] - code - apps/web/app/[locale]/(tenant)/tenant/assessments/page.tsx
- [[GameQuizPlayer()]] - code - apps/web/components/learner/game-quiz-player.tsx
- [[GameStage()]] - code - apps/web/components/learner/game-quiz-player.tsx
- [[GradingToggle()]] - code - apps/web/app/[locale]/(tenant)/tenant/assessments/page.tsx
- [[Leaderboard()]] - code - apps/web/app/[locale]/(learner)/learner/assessments/page.tsx
- [[LeaderboardTable()]] - code - apps/web/components/learner/leaderboard-table.tsx
- [[LearnerAssessment]] - code - packages/types/index.ts
- [[LearnerAssessmentsPage()]] - code - apps/web/app/[locale]/(learner)/learner/assessments/page.tsx
- [[LiveGameRow()]] - code - apps/web/app/[locale]/(tenant)/tenant/assessments/page.tsx
- [[Phase]] - code - apps/web/components/learner/game-quiz-player.tsx
- [[QUESTION_TYPES]] - code - apps/web/components/tenant/question-editor.tsx
- [[QUESTION_TYPE_LABEL_KEYS]] - code - apps/web/components/tenant/question-editor.tsx
- [[QuestionBank]] - code - packages/types/index.ts
- [[QuestionBody]] - code - apps/web/components/tenant/question-editor.tsx
- [[QuestionDifficulty]] - code - packages/types/index.ts
- [[QuestionEditor()]] - code - apps/web/components/tenant/question-editor.tsx
- [[ResultsSection()]] - code - apps/web/app/[locale]/(tenant)/tenant/assessments/page.tsx
- [[SIMPLE_ANSWER_TYPES]] - code - apps/web/components/tenant/question-editor.tsx
- [[STATUS_FILTERS_1]] - code - apps/web/app/[locale]/(tenant)/tenant/assessments/page.tsx
- [[STEP_LABEL_KEYS]] - code - apps/web/app/[locale]/(tenant)/tenant/assessments/page.tsx
- [[StringListField()]] - code - apps/web/components/tenant/question-editor.tsx
- [[TenantAssessment]] - code - packages/types/index.ts
- [[TenantAssessmentsPage()]] - code - apps/web/app/[locale]/(tenant)/tenant/assessments/page.tsx
- [[WIZARD_STEPS]] - code - apps/web/app/[locale]/(tenant)/tenant/assessments/page.tsx
- [[WizardStep]] - code - apps/web/app/[locale]/(tenant)/tenant/assessments/page.tsx
- [[WrittenForm()]] - code - apps/web/app/[locale]/(learner)/learner/assessments/page.tsx
- [[asStringArray()]] - code - apps/web/components/tenant/question-editor.tsx
- [[clozeOptions()]] - code - apps/web/components/learner/game-quiz-player.tsx
- [[dragItems()]] - code - apps/web/components/learner/game-quiz-player.tsx
- [[dragTargets()]] - code - apps/web/components/learner/game-quiz-player.tsx
- [[fillBlankCount()]] - code - apps/web/components/learner/game-quiz-player.tsx
- [[game-quiz-player.tsx]] - code - apps/web/components/learner/game-quiz-player.tsx
- [[hotspotImageUrl()]] - code - apps/web/components/learner/game-quiz-player.tsx
- [[leaderboard-table.tsx]] - code - apps/web/components/learner/leaderboard-table.tsx
- [[matchingLeft()]] - code - apps/web/components/learner/game-quiz-player.tsx
- [[mutErr()]] - code - apps/web/app/[locale]/(tenant)/tenant/assessments/page.tsx
- [[mutErr()_1]] - code - apps/web/components/tenant/question-editor.tsx
- [[page.tsx_15]] - code - apps/web/app/[locale]/(learner)/learner/assessments/page.tsx
- [[page.tsx_19]] - code - apps/web/app/[locale]/(tenant)/tenant/assessments/page.tsx
- [[question-editor.tsx]] - code - apps/web/components/tenant/question-editor.tsx
- [[toDatetimeLocal()]] - code - apps/web/app/[locale]/(tenant)/tenant/assessments/page.tsx
- [[useAssessmentLeaderboard()]] - code - apps/web/hooks/useAssessments.ts
- [[useAssessmentResults()]] - code - apps/web/hooks/useAssessments.ts
- [[useAssessments.ts]] - code - apps/web/hooks/useAssessments.ts
- [[useAssignAssessment()]] - code - apps/web/hooks/useAssessments.ts
- [[useBankQuestions()]] - code - apps/web/hooks/useAssessments.ts
- [[useCreateAssessment()]] - code - apps/web/hooks/useAssessments.ts
- [[useCreateBankQuestion()]] - code - apps/web/hooks/useAssessments.ts
- [[useCreateQuestionBank()]] - code - apps/web/hooks/useAssessments.ts
- [[useGenerateBankQuestions()]] - code - apps/web/hooks/useAssessments.ts
- [[useLearnerAssessments()]] - code - apps/web/hooks/useAssessments.ts
- [[useLearnerLeaderboard()]] - code - apps/web/hooks/useAssessments.ts
- [[usePatchBankQuestion()]] - code - apps/web/hooks/useAssessments.ts
- [[useQuestionBanks()]] - code - apps/web/hooks/useAssessments.ts
- [[useScheduleAssessment()]] - code - apps/web/hooks/useAssessments.ts
- [[useSetAssessmentLive()]] - code - apps/web/hooks/useAssessments.ts
- [[useSubmitLearnerAssessment()]] - code - apps/web/hooks/useAssessments.ts

## Live Query (requires Dataview plugin)

```dataview
TABLE source_file, type FROM #community/Assessments__Game_Quiz_UI
SORT file.name ASC
```

## Connections to other communities
- 18 edges to [[_COMMUNITY_Shared Types & Locale]]
- 14 edges to [[_COMMUNITY_Admin Panel UI]]
- 11 edges to [[_COMMUNITY_Marketing Landing Page]]
- 7 edges to [[_COMMUNITY_Tenant Pages & Billing UI]]
- 7 edges to [[_COMMUNITY_Practice & Content Dialogs]]
- 6 edges to [[_COMMUNITY_Message Bells & Threads]]
- 6 edges to [[_COMMUNITY_Content Hooks & Status]]
- 4 edges to [[_COMMUNITY_Shared UI Primitives]]
- 3 edges to [[_COMMUNITY_Learner Dashboard]]
- 3 edges to [[_COMMUNITY_Tenant Students & Messaging UI]]
- 2 edges to [[_COMMUNITY_Content Stage & Limits]]
- 2 edges to [[_COMMUNITY_Question Generation Pipeline]]
- 2 edges to [[_COMMUNITY_Quiz UI & Rich Text]]

## Top bridge nodes
- [[page.tsx_19]] - degree 51, connects to 10 communities
- [[useAssessments.ts]] - degree 40, connects to 6 communities
- [[page.tsx_15]] - degree 22, connects to 5 communities
- [[game-quiz-player.tsx]] - degree 23, connects to 4 communities
- [[TenantAssessmentsPage()]] - degree 14, connects to 4 communities