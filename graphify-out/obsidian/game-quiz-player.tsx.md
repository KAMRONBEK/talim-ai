---
source_file: "apps/web/components/learner/game-quiz-player.tsx"
type: "code"
community: "Community None"
location: "L1"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/Community_None
---

# game-quiz-player.tsx

## Connections
- [[ANSWER_CHIPS]] - `contains` [EXTRACTED]
- [[AssessmentSubmitResult]] - `imports` [EXTRACTED]
- [[Button]] - `imports` [EXTRACTED]
- [[GameQuizPlayer()]] - `contains` [EXTRACTED]
- [[GameStage()]] - `contains` [EXTRACTED]
- [[Input]] - `imports` [EXTRACTED]
- [[LearnerAssessment]] - `imports` [EXTRACTED]
- [[LogoMark()_1]] - `imports` [EXTRACTED]
- [[Phase]] - `contains` [EXTRACTED]
- [[RichText()]] - `imports` [EXTRACTED]
- [[clozeOptions()]] - `contains` [EXTRACTED]
- [[dragItems()]] - `contains` [EXTRACTED]
- [[dragTargets()]] - `contains` [EXTRACTED]
- [[fillBlankCount()]] - `contains` [EXTRACTED]
- [[hotspotImageUrl()]] - `contains` [EXTRACTED]
- [[index.ts_2]] - `imports_from` [EXTRACTED]
- [[index.ts_3]] - `imports_from` [EXTRACTED]
- [[logo.tsx_1]] - `imports_from` [EXTRACTED]
- [[matchingLeft()]] - `contains` [EXTRACTED]
- [[page.tsx_15]] - `imports_from` [EXTRACTED]
- [[rich-text.tsx]] - `imports_from` [EXTRACTED]
- [[useAssessments.ts]] - `imports_from` [EXTRACTED]
- [[useSubmitLearnerAssessment()]] - `imports` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/Community_None