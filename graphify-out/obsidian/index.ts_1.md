---
source_file: "apps/api/src/routes/index.ts"
type: "code"
community: "src routes"
location: "L1"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/src_routes
---

# index.ts

## Connections
- [[admin.routes.ts]] - `imports_from` [EXTRACTED]
- [[adminRoutes]] - `imports` [EXTRACTED]
- [[auth.routes.ts]] - `imports_from` [EXTRACTED]
- [[authRoutes]] - `imports` [EXTRACTED]
- [[billing.routes.ts]] - `imports_from` [EXTRACTED]
- [[billingRoutes]] - `imports` [EXTRACTED]
- [[chat.routes.ts]] - `imports_from` [EXTRACTED]
- [[chatRoutes]] - `imports` [EXTRACTED]
- [[content.routes.ts]] - `imports_from` [EXTRACTED]
- [[contentRoutes]] - `imports` [EXTRACTED]
- [[events.routes.ts]] - `imports_from` [EXTRACTED]
- [[eventsRoutes]] - `imports` [EXTRACTED]
- [[index.ts]] - `imports_from` [EXTRACTED]
- [[learner.routes.ts]] - `imports_from` [EXTRACTED]
- [[learnerRoutes]] - `imports` [EXTRACTED]
- [[quiz.routes.ts]] - `imports_from` [EXTRACTED]
- [[quizRoutes]] - `imports` [EXTRACTED]
- [[routes]] - `contains` [EXTRACTED]
- [[summary.routes.ts]] - `imports_from` [EXTRACTED]
- [[summaryRoutes]] - `imports` [EXTRACTED]
- [[tenant.routes.ts]] - `imports_from` [EXTRACTED]
- [[tenantRoutes]] - `imports` [EXTRACTED]
- [[usage.routes.ts]] - `imports_from` [EXTRACTED]
- [[usageRoutes]] - `imports` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/src_routes