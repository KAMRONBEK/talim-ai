---
source_file: "apps/api/src/services/answerJudge.service.ts"
type: "code"
community: "Quiz Controller & Grading"
location: "L1"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/Quiz_Controller__Grading
---

# answerJudge.service.ts

## Connections
- [[AiGradeEntry]] - `contains` [EXTRACTED]
- [[Env]] - `imports` [EXTRACTED]
- [[JudgeCandidate]] - `contains` [EXTRACTED]
- [[JudgeOptions]] - `contains` [EXTRACTED]
- [[JudgeVerdict]] - `contains` [EXTRACTED]
- [[ai.service.ts]] - `imports_from` [EXTRACTED]
- [[applyAiJudgeToGrades()]] - `contains` [EXTRACTED]
- [[bankQuestionKey()]] - `contains` [EXTRACTED]
- [[consumeJudgeBudget()]] - `contains` [EXTRACTED]
- [[env.ts]] - `imports_from` [EXTRACTED]
- [[generateJsonCompletion()]] - `imports` [EXTRACTED]
- [[index.ts_2]] - `imports_from` [EXTRACTED]
- [[isAiJudgedQuestionType()]] - `imports` [EXTRACTED]
- [[isJudgeable()]] - `contains` [EXTRACTED]
- [[judgeBudgets]] - `contains` [EXTRACTED]
- [[judgeWithModel()]] - `contains` [EXTRACTED]
- [[judgeWrittenAnswers()]] - `contains` [EXTRACTED]
- [[learner.ts]] - `imports_from` [EXTRACTED]
- [[learningProgress.service.ts]] - `imports_from` [EXTRACTED]
- [[normalizeAnswer()]] - `imports` [EXTRACTED]
- [[prisma_2]] - `imports` [EXTRACTED]
- [[prisma.ts]] - `imports_from` [EXTRACTED]
- [[quiz.controller.ts]] - `imports_from` [EXTRACTED]
- [[quizQuestionKey()]] - `contains` [EXTRACTED]
- [[verdictHash()]] - `contains` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/Quiz_Controller__Grading