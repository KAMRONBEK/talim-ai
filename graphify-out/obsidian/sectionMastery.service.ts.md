---
source_file: "apps/api/src/services/sectionMastery.service.ts"
type: "code"
community: "Section Mastery & Elo"
location: "L1"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/Section_Mastery__Elo
---

# sectionMastery.service.ts

## Connections
- [[AnswerEvidence]] - `contains` [EXTRACTED]
- [[ContentMasteryResponse]] - `imports` [EXTRACTED]
- [[MasteryDelta]] - `imports` [EXTRACTED]
- [[SectionMasteryInfo]] - `imports` [EXTRACTED]
- [[dayStamp()]] - `contains` [EXTRACTED]
- [[decayedTheta()]] - `imports` [EXTRACTED]
- [[difficultyPrior()]] - `imports` [EXTRACTED]
- [[eloUpdate()]] - `imports` [EXTRACTED]
- [[flashcardEvidence()]] - `imports` [EXTRACTED]
- [[flashcards.controller.ts]] - `imports_from` [EXTRACTED]
- [[getContentMastery()_1]] - `contains` [EXTRACTED]
- [[guessFloorForQuestion()]] - `imports` [EXTRACTED]
- [[index.ts_2]] - `imports_from` [EXTRACTED]
- [[learner.ts]] - `imports_from` [EXTRACTED]
- [[masteryScore()]] - `imports` [EXTRACTED]
- [[prisma_2]] - `imports` [EXTRACTED]
- [[prisma.ts]] - `imports_from` [EXTRACTED]
- [[quiz.controller.ts]] - `imports_from` [EXTRACTED]
- [[recordAnswers()]] - `contains` [EXTRACTED]
- [[recordFlashcardReview()]] - `contains` [EXTRACTED]
- [[resolveMasteryBand()]] - `imports` [EXTRACTED]
- [[scopeKeyFor()]] - `contains` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/Section_Mastery__Elo