---
source_file: "apps/api/src/controllers/admin/content.controller.ts"
type: "code"
community: "Admin Content & Audit Controllers"
location: "L1"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/Admin_Content__Audit_Controllers
---

# content.controller.ts

## Connections
- [[AppError]] - `imports` [EXTRACTED]
- [[AuthenticatedRequest]] - `imports` [EXTRACTED]
- [[GENERATED_KINDS]] - `contains` [EXTRACTED]
- [[StorageService]] - `imports` [EXTRACTED]
- [[admin.controller.ts]] - `re_exports` [EXTRACTED]
- [[analytics.service.ts]] - `imports_from` [EXTRACTED]
- [[audit.service.ts]] - `imports_from` [EXTRACTED]
- [[auth.middleware.ts]] - `imports_from` [EXTRACTED]
- [[cancelContentJobs()]] - `imports` [EXTRACTED]
- [[contentDetail()]] - `contains` [EXTRACTED]
- [[contentQueue]] - `imports` [EXTRACTED]
- [[deleteContent()]] - `contains` [EXTRACTED]
- [[deleteGenerated()]] - `contains` [EXTRACTED]
- [[error.middleware.ts]] - `imports_from` [EXTRACTED]
- [[getChunkSample()]] - `imports` [EXTRACTED]
- [[getParam()]] - `imports` [EXTRACTED]
- [[listContents()]] - `contains` [EXTRACTED]
- [[listGenerated()]] - `contains` [EXTRACTED]
- [[paginationSchema]] - `imports` [EXTRACTED]
- [[params.ts]] - `imports_from` [EXTRACTED]
- [[prisma_2]] - `imports` [EXTRACTED]
- [[prisma.ts]] - `imports_from` [EXTRACTED]
- [[queue.service.ts]] - `imports_from` [EXTRACTED]
- [[retryContentJob()]] - `contains` [EXTRACTED]
- [[reviewBodySchema]] - `contains` [EXTRACTED]
- [[reviewGenerated()]] - `contains` [EXTRACTED]
- [[shared.ts]] - `imports_from` [EXTRACTED]
- [[storage.service.ts]] - `imports_from` [EXTRACTED]
- [[writeAdminAuditLog()]] - `imports` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/Admin_Content__Audit_Controllers