---
source_file: "apps/api/src/services/admin/analytics.service.ts"
type: "code"
community: "Community 51"
location: "L1"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/Community_51
---

# analytics.service.ts

## Connections
- [[ALL_CONTENT_TYPES]] - `contains` [EXTRACTED]
- [[ALL_ROLES]] - `contains` [EXTRACTED]
- [[AdminAnalyticsSummary]] - `imports` [EXTRACTED]
- [[AdminChunkSample]] - `contains` [EXTRACTED]
- [[AdminContentByTypeResponse]] - `imports` [EXTRACTED]
- [[AdminFunnelResponse]] - `imports` [EXTRACTED]
- [[AdminMrrResponse]] - `imports` [EXTRACTED]
- [[AdminSpendByModelResponse]] - `imports` [EXTRACTED]
- [[AdminTopOrgsResponse]] - `imports` [EXTRACTED]
- [[AdminUserGrowthResponse]] - `imports` [EXTRACTED]
- [[AdminUsersByRoleResponse]] - `imports` [EXTRACTED]
- [[analytics.controller.ts]] - `imports_from` [EXTRACTED]
- [[computeMrr()]] - `contains` [EXTRACTED]
- [[content.controller.ts]] - `imports_from` [EXTRACTED]
- [[daysAgo()]] - `contains` [EXTRACTED]
- [[estimateTokenCostUsd()]] - `imports` [EXTRACTED]
- [[getAnalyticsSummary()]] - `contains` [EXTRACTED]
- [[getChunkSample()]] - `contains` [EXTRACTED]
- [[getContentByType()]] - `contains` [EXTRACTED]
- [[getFunnel()]] - `contains` [EXTRACTED]
- [[getSpendByModel()]] - `contains` [EXTRACTED]
- [[getTopOrgs()]] - `contains` [EXTRACTED]
- [[getUserGrowth()]] - `contains` [EXTRACTED]
- [[getUsersByRole()]] - `contains` [EXTRACTED]
- [[index.ts_2]] - `imports_from` [EXTRACTED]
- [[planMonthlyPriceUsd()]] - `imports` [EXTRACTED]
- [[prisma_2]] - `imports` [EXTRACTED]
- [[prisma.ts]] - `imports_from` [EXTRACTED]
- [[usage-pricing.ts]] - `imports_from` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/Community_51