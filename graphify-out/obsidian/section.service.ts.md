---
source_file: "apps/api/src/services/section.service.ts"
type: "code"
community: "Community None"
location: "L1"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/Community_None
---

# section.service.ts

## Connections
- [[AppLocale]] - `imports` [EXTRACTED]
- [[GeneratedSection]] - `contains` [EXTRACTED]
- [[GeneratedSubsection]] - `contains` [EXTRACTED]
- [[SECTION_TITLE_LOCALE_PROMPT]] - `contains` [EXTRACTED]
- [[SectionTitleInput]] - `contains` [EXTRACTED]
- [[ai.service.ts]] - `imports_from` [EXTRACTED]
- [[buildSectionUserPrompt()]] - `imports` [EXTRACTED]
- [[ensureSectionTitlesForLocale()]] - `contains` [EXTRACTED]
- [[generateContentSections()]] - `contains` [EXTRACTED]
- [[generateJsonCompletion()]] - `imports` [EXTRACTED]
- [[getSectionBody()]] - `contains` [EXTRACTED]
- [[index.ts_2]] - `imports_from` [EXTRACTED]
- [[ingest.service.ts]] - `imports_from` [EXTRACTED]
- [[learningProgress.service.ts]] - `imports_from` [EXTRACTED]
- [[prisma_3]] - `imports` [EXTRACTED]
- [[prisma.ts]] - `imports_from` [EXTRACTED]
- [[processContent.job.ts]] - `imports_from` [EXTRACTED]
- [[resolveSectionTitle()]] - `contains` [EXTRACTED]
- [[section-prompt.ts]] - `imports_from` [EXTRACTED]
- [[section.controller.ts]] - `imports_from` [EXTRACTED]
- [[slides.service.ts]] - `imports_from` [EXTRACTED]
- [[summary.controller.ts]] - `imports_from` [EXTRACTED]
- [[translateSectionTitles()]] - `contains` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/Community_None