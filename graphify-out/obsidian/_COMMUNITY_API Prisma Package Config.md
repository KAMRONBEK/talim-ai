---
type: community
cohesion: 0.29
members: 7
---

# API Prisma Package Config

**Cohesion:** 0.29 - loosely connected
**Members:** 7 nodes

## Members
- [[name_1]] - code - apps/api/package.json
- [[package.json_1]] - code - apps/api/package.json
- [[prisma_1]] - code - apps/api/package.json
- [[private_1]] - code - apps/api/package.json
- [[schema]] - code - apps/api/package.json
- [[seed]] - code - apps/api/package.json
- [[version_1]] - code - apps/api/package.json

## Live Query (requires Dataview plugin)

```dataview
TABLE source_file, type FROM #community/API_Prisma_Package_Config
SORT file.name ASC
```

## Connections to other communities
- 1 edge to [[_COMMUNITY_API Dependencies]]
- 1 edge to [[_COMMUNITY_API Dev Tooling]]
- 1 edge to [[_COMMUNITY_Root pnpm Scripts]]

## Top bridge nodes
- [[package.json_1]] - degree 7, connects to 3 communities