---
source_file: "apps/api/src/controllers/admin/users.controller.ts"
type: "code"
community: "controllers admin"
location: "L1"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/controllers_admin
---

# users.controller.ts

## Connections
- [[AppError]] - `imports` [EXTRACTED]
- [[AuthenticatedRequest]] - `imports` [EXTRACTED]
- [[admin.controller.ts]] - `re_exports` [EXTRACTED]
- [[adminUpdateUserSubscription()]] - `imports` [EXTRACTED]
- [[adminUserRole.service.ts]] - `imports_from` [EXTRACTED]
- [[applyAdminRoleChange()]] - `imports` [EXTRACTED]
- [[audit.service.ts]] - `imports_from` [EXTRACTED]
- [[auth.middleware.ts]] - `imports_from` [EXTRACTED]
- [[buildUserWhere()]] - `imports` [EXTRACTED]
- [[contentAccess.service.ts]] - `imports_from` [EXTRACTED]
- [[createUser()]] - `contains` [EXTRACTED]
- [[createUserSchema]] - `contains` [EXTRACTED]
- [[deleteUser()]] - `contains` [EXTRACTED]
- [[deleteUserSchema]] - `contains` [EXTRACTED]
- [[error.middleware.ts]] - `imports_from` [EXTRACTED]
- [[formatAdminUser()]] - `imports` [EXTRACTED]
- [[generateTemporaryPassword()]] - `imports` [EXTRACTED]
- [[getParam()]] - `imports` [EXTRACTED]
- [[getSubscriptionForTenant()]] - `imports` [EXTRACTED]
- [[getSubscriptionForUser()]] - `imports` [EXTRACTED]
- [[getTenantUsageVsLimits()]] - `imports` [EXTRACTED]
- [[getUsageForPeriod()]] - `imports` [EXTRACTED]
- [[getUsageVsLimits()]] - `imports` [EXTRACTED]
- [[getUser()]] - `contains` [EXTRACTED]
- [[listUsers()]] - `contains` [EXTRACTED]
- [[paginationSchema]] - `imports` [EXTRACTED]
- [[params.ts]] - `imports_from` [EXTRACTED]
- [[patchSubscriptionSchema]] - `contains` [EXTRACTED]
- [[patchUser()]] - `contains` [EXTRACTED]
- [[patchUserSchema]] - `contains` [EXTRACTED]
- [[patchUserSubscription()]] - `contains` [EXTRACTED]
- [[prisma_2]] - `imports` [EXTRACTED]
- [[prisma.ts]] - `imports_from` [EXTRACTED]
- [[resetPasswordSchema]] - `contains` [EXTRACTED]
- [[resetUserPassword()]] - `contains` [EXTRACTED]
- [[resolveTenantIdForUser()]] - `imports` [EXTRACTED]
- [[shared.ts]] - `imports_from` [EXTRACTED]
- [[subscription.service.ts]] - `imports_from` [EXTRACTED]
- [[usage.service.ts]] - `imports_from` [EXTRACTED]
- [[writeAdminAuditLog()]] - `imports` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/controllers_admin