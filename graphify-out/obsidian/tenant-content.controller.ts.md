---
source_file: "apps/api/src/controllers/tenant-content.controller.ts"
type: "code"
community: "Tenant Content Controller"
location: "L1"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/Tenant_Content_Controller
---

# tenant-content.controller.ts

## Connections
- [[AppError]] - `imports` [EXTRACTED]
- [[AuthenticatedRequest]] - `imports` [EXTRACTED]
- [[Env]] - `imports` [EXTRACTED]
- [[StorageService]] - `imports` [EXTRACTED]
- [[assertQuota()]] - `imports` [EXTRACTED]
- [[assertTenantOwnsContent()]] - `imports` [EXTRACTED]
- [[auth.middleware.ts]] - `imports_from` [EXTRACTED]
- [[autoGenerateSectionDecks()]] - `imports` [EXTRACTED]
- [[cancelContentJobs()]] - `imports` [EXTRACTED]
- [[captionAndStoreFigures()]] - `imports` [EXTRACTED]
- [[contentAccess.service.ts]] - `imports_from` [EXTRACTED]
- [[contentQueue]] - `imports` [EXTRACTED]
- [[createYoutubeContent()_1]] - `contains` [EXTRACTED]
- [[deleteContent()_2]] - `contains` [EXTRACTED]
- [[env.ts]] - `imports_from` [EXTRACTED]
- [[error.middleware.ts]] - `imports_from` [EXTRACTED]
- [[extractRegionTextFromImage()]] - `imports` [EXTRACTED]
- [[extractTextFromPageImages()]] - `imports` [EXTRACTED]
- [[extractYoutubeTranscript()]] - `imports` [EXTRACTED]
- [[extractYoutubeVideoId()]] - `imports` [EXTRACTED]
- [[figure.service.ts]] - `imports_from` [EXTRACTED]
- [[formatContent()_1]] - `contains` [EXTRACTED]
- [[formatTranscriptSegment()_1]] - `contains` [EXTRACTED]
- [[getContent()_1]] - `contains` [EXTRACTED]
- [[getContentFile()_1]] - `contains` [EXTRACTED]
- [[getContentTranscript()_1]] - `contains` [EXTRACTED]
- [[getParam()]] - `imports` [EXTRACTED]
- [[ingest.service.ts]] - `imports_from` [EXTRACTED]
- [[ingestText()]] - `imports` [EXTRACTED]
- [[listContent()_1]] - `contains` [EXTRACTED]
- [[ocrPdfRegion()_1]] - `contains` [EXTRACTED]
- [[ocrRegionSchema_1]] - `contains` [EXTRACTED]
- [[params.ts]] - `imports_from` [EXTRACTED]
- [[pdf.service.ts]] - `imports_from` [EXTRACTED]
- [[prisma_2]] - `imports` [EXTRACTED]
- [[prisma.ts]] - `imports_from` [EXTRACTED]
- [[queue.service.ts]] - `imports_from` [EXTRACTED]
- [[reparseContent()_1]] - `contains` [EXTRACTED]
- [[reparseSchema_1]] - `contains` [EXTRACTED]
- [[requireTenantId()]] - `contains` [EXTRACTED]
- [[retryContent()_1]] - `contains` [EXTRACTED]
- [[slides.service.ts]] - `imports_from` [EXTRACTED]
- [[storage.service.ts]] - `imports_from` [EXTRACTED]
- [[subscription.service.ts]] - `imports_from` [EXTRACTED]
- [[tenant.routes.ts]] - `imports_from` [EXTRACTED]
- [[uploadContent()_1]] - `contains` [EXTRACTED]
- [[youtube.service.ts]] - `imports_from` [EXTRACTED]
- [[youtubeSchema_1]] - `contains` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/Tenant_Content_Controller