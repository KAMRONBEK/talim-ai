---
source_file: "apps/api/src/lib/question-gen.ts"
type: "code"
community: "Quiz Generation Pipeline"
location: "L1"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/Quiz_Generation_Pipeline
---

# question-gen.ts

## Connections
- [[AppLocale]] - `imports` [EXTRACTED]
- [[GeneratableQuestionType]] - `imports` [EXTRACTED]
- [[GenerateOptions]] - `contains` [EXTRACTED]
- [[GeneratedQuestion]] - `imports` [EXTRACTED]
- [[ProcessedQuestion]] - `imports` [EXTRACTED]
- [[QuestionDepth]] - `imports` [EXTRACTED]
- [[QuestionSetRequest]] - `contains` [EXTRACTED]
- [[QuestionSetResult]] - `contains` [EXTRACTED]
- [[SkipReason]] - `imports` [EXTRACTED]
- [[ai.service.ts]] - `imports_from` [EXTRACTED]
- [[banks.ts]] - `imports_from` [EXTRACTED]
- [[buildQuestionGenPrompt()]] - `imports` [EXTRACTED]
- [[generateJsonCompletion()]] - `imports` [EXTRACTED]
- [[generateQuestionSet()]] - `contains` [EXTRACTED]
- [[generateQuiz.job.ts]] - `imports_from` [EXTRACTED]
- [[getQuestionGenSystemPrompt()]] - `imports` [EXTRACTED]
- [[index.ts_2]] - `imports_from` [EXTRACTED]
- [[overgenerateCount()]] - `imports` [EXTRACTED]
- [[postprocessGeneratedQuestions()]] - `imports` [EXTRACTED]
- [[question-gen-prompt.ts]] - `imports_from` [EXTRACTED]
- [[question-postprocess.ts]] - `imports_from` [EXTRACTED]
- [[shared.ts_1]] - `imports_from` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/Quiz_Generation_Pipeline