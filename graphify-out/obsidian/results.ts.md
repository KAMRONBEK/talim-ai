---
source_file: "apps/api/src/services/assessment/results.ts"
type: "code"
community: "Community 51"
location: "L1"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/Community_51
---

# results.ts

## Connections
- [[AppError]] - `imports` [EXTRACTED]
- [[LeaderboardAttempt]] - `imports` [EXTRACTED]
- [[assessment.service.ts]] - `re_exports` [EXTRACTED]
- [[error.middleware.ts]] - `imports_from` [EXTRACTED]
- [[getAssessmentLeaderboard()]] - `contains` [EXTRACTED]
- [[getAssessmentResults()]] - `contains` [EXTRACTED]
- [[learner.ts]] - `imports_from` [EXTRACTED]
- [[learnerDisplayName()]] - `imports` [EXTRACTED]
- [[prisma_2]] - `imports` [EXTRACTED]
- [[prisma.ts]] - `imports_from` [EXTRACTED]
- [[shared.ts_1]] - `imports_from` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/Community_51