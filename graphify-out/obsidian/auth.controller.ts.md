---
source_file: "apps/api/src/controllers/auth.controller.ts"
type: "code"
community: "Auth Controller (login/join/password)"
location: "L1"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/Auth_Controller_login/join/password
---

# auth.controller.ts

## Connections
- [[AppError]] - `imports` [EXTRACTED]
- [[AuthenticatedRequest]] - `imports` [EXTRACTED]
- [[Env]] - `imports` [EXTRACTED]
- [[auth.middleware.ts]] - `imports_from` [EXTRACTED]
- [[auth.routes.ts]] - `imports_from` [EXTRACTED]
- [[changePassword()]] - `contains` [EXTRACTED]
- [[changePasswordSchema]] - `contains` [EXTRACTED]
- [[contentAccess.service.ts]] - `imports_from` [EXTRACTED]
- [[createTutorRequest()]] - `imports` [EXTRACTED]
- [[env.ts]] - `imports_from` [EXTRACTED]
- [[error.middleware.ts]] - `imports_from` [EXTRACTED]
- [[formatUser()]] - `contains` [EXTRACTED]
- [[getMyLatestTutorRequest()]] - `imports` [EXTRACTED]
- [[getTutorRequest()]] - `contains` [EXTRACTED]
- [[index.ts_2]] - `imports_from` [EXTRACTED]
- [[joinClass()]] - `contains` [EXTRACTED]
- [[joinSchema]] - `contains` [EXTRACTED]
- [[joinTenantByCode()]] - `imports` [EXTRACTED]
- [[login()]] - `contains` [EXTRACTED]
- [[loginSchema]] - `contains` [EXTRACTED]
- [[me()]] - `contains` [EXTRACTED]
- [[parseAppLocale()]] - `imports` [EXTRACTED]
- [[prisma_2]] - `imports` [EXTRACTED]
- [[prisma.ts]] - `imports_from` [EXTRACTED]
- [[register()]] - `contains` [EXTRACTED]
- [[registerSchema]] - `contains` [EXTRACTED]
- [[registerTenant()]] - `contains` [EXTRACTED]
- [[resolveTenantIdForUser()]] - `imports` [EXTRACTED]
- [[signToken()]] - `contains` [EXTRACTED]
- [[tenant.service.ts]] - `imports_from` [EXTRACTED]
- [[tutorRequest.service.ts]] - `imports_from` [EXTRACTED]
- [[updateMe()]] - `contains` [EXTRACTED]
- [[updateMeSchema]] - `contains` [EXTRACTED]
- [[upgradeToTenant()]] - `contains` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/Auth_Controller_login/join/password