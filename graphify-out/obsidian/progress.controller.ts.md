---
source_file: "apps/api/src/controllers/progress.controller.ts"
type: "code"
community: "Media & Progress Controllers"
location: "L1"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/Media__Progress_Controllers
---

# progress.controller.ts

## Connections
- [[AppError]] - `imports` [EXTRACTED]
- [[AuthenticatedRequest]] - `imports` [EXTRACTED]
- [[assertCanAccessContent()]] - `imports` [EXTRACTED]
- [[auth.middleware.ts]] - `imports_from` [EXTRACTED]
- [[computeStreakDays()]] - `imports` [EXTRACTED]
- [[content.routes.ts]] - `imports_from` [EXTRACTED]
- [[contentAccess.service.ts]] - `imports_from` [EXTRACTED]
- [[episodeProgressSchema]] - `contains` [EXTRACTED]
- [[error.middleware.ts]] - `imports_from` [EXTRACTED]
- [[formatSectionProgress()]] - `contains` [EXTRACTED]
- [[getContentProgress()]] - `contains` [EXTRACTED]
- [[getEpisodeProgress()]] - `contains` [EXTRACTED]
- [[getLearningHistory()]] - `contains` [EXTRACTED]
- [[getParam()]] - `imports` [EXTRACTED]
- [[learningProgress.service.ts]] - `imports_from` [EXTRACTED]
- [[locale.ts]] - `imports_from` [EXTRACTED]
- [[markSectionViewed()]] - `imports` [EXTRACTED]
- [[params.ts]] - `imports_from` [EXTRACTED]
- [[patchContentProgress()]] - `contains` [EXTRACTED]
- [[patchEpisodeProgress()]] - `contains` [EXTRACTED]
- [[patchProgressSchema]] - `contains` [EXTRACTED]
- [[prisma_2]] - `imports` [EXTRACTED]
- [[prisma.ts]] - `imports_from` [EXTRACTED]
- [[resolveLocale()]] - `imports` [EXTRACTED]
- [[tenant.routes.ts]] - `imports_from` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/Media__Progress_Controllers