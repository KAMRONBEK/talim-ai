---
source_file: "apps/api/src/jobs/generateQuiz.job.ts"
type: "code"
community: "Quiz Generation Pipeline"
location: "L1"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/Quiz_Generation_Pipeline
---

# generateQuiz.job.ts

## Connections
- [[ContextChunk]] - `contains` [EXTRACTED]
- [[GENERATABLE_TYPES]] - `imports` [EXTRACTED]
- [[GeneratableQuestionType]] - `imports` [EXTRACTED]
- [[GenerateQuizJobData]] - `imports` [EXTRACTED]
- [[QuestionDepth]] - `imports` [EXTRACTED]
- [[QuestionStyle]] - `imports` [EXTRACTED]
- [[assessment-prompt.ts]] - `imports_from` [EXTRACTED]
- [[buildRagContext()]] - `imports` [EXTRACTED]
- [[chunk-sampling.ts]] - `imports_from` [EXTRACTED]
- [[containmentNormalize()]] - `contains` [EXTRACTED]
- [[generateQuestionSet()]] - `imports` [EXTRACTED]
- [[getSectionChunks()]] - `contains` [EXTRACTED]
- [[index.ts]] - `imports_from` [EXTRACTED]
- [[index.ts_2]] - `imports_from` [EXTRACTED]
- [[jobEventAudience.ts]] - `imports_from` [EXTRACTED]
- [[normalizePracticeQuestionType()]] - `imports` [EXTRACTED]
- [[parseAppLocale()]] - `imports` [EXTRACTED]
- [[prisma_2]] - `imports` [EXTRACTED]
- [[prisma.ts]] - `imports_from` [EXTRACTED]
- [[publishContentEvent()]] - `imports` [EXTRACTED]
- [[question-gen-prompt.ts]] - `imports_from` [EXTRACTED]
- [[question-gen.ts]] - `imports_from` [EXTRACTED]
- [[queue.service.ts]] - `imports_from` [EXTRACTED]
- [[quizQueue]] - `imports` [EXTRACTED]
- [[rag.service.ts]] - `imports_from` [EXTRACTED]
- [[registerGenerateQuizJob()]] - `contains` [EXTRACTED]
- [[resolveRequestedTypes()]] - `contains` [EXTRACTED]
- [[resolveSourceSection()]] - `contains` [EXTRACTED]
- [[sampleChunksEvenly()]] - `imports` [EXTRACTED]
- [[searchSimilarChunks()]] - `imports` [EXTRACTED]
- [[typesFromStyle()]] - `imports` [EXTRACTED]
- [[wholeMaterialTarget()]] - `contains` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/Quiz_Generation_Pipeline