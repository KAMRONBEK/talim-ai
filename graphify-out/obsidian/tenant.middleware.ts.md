---
source_file: "apps/api/src/middleware/tenant.middleware.ts"
type: "code"
community: "API Middleware (auth/quota/rate-limit)"
location: "L1"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/API_Middleware_auth/quota/rate-limit
---

# tenant.middleware.ts

## Connections
- [[AppError]] - `imports` [EXTRACTED]
- [[AuthenticatedRequest]] - `imports` [EXTRACTED]
- [[attachTenantId()]] - `contains` [EXTRACTED]
- [[auth.middleware.ts]] - `imports_from` [EXTRACTED]
- [[billing.routes.ts]] - `imports_from` [EXTRACTED]
- [[blockIndividualContentForOwner()]] - `contains` [EXTRACTED]
- [[blockLearnerMutations()]] - `contains` [EXTRACTED]
- [[chat.routes.ts]] - `imports_from` [EXTRACTED]
- [[content.routes.ts]] - `imports_from` [EXTRACTED]
- [[contentAccess.service.ts]] - `imports_from` [EXTRACTED]
- [[error.middleware.ts]] - `imports_from` [EXTRACTED]
- [[learner.routes.ts]] - `imports_from` [EXTRACTED]
- [[prisma_2]] - `imports` [EXTRACTED]
- [[prisma.ts]] - `imports_from` [EXTRACTED]
- [[quiz.routes.ts]] - `imports_from` [EXTRACTED]
- [[requireActiveLearner()]] - `contains` [EXTRACTED]
- [[requireTenantId()_1]] - `contains` [EXTRACTED]
- [[requireTenantMember()]] - `contains` [EXTRACTED]
- [[requireTenantOwner()]] - `contains` [EXTRACTED]
- [[resolveTenantIdForUser()]] - `imports` [EXTRACTED]
- [[summary.routes.ts]] - `imports_from` [EXTRACTED]
- [[tenant.routes.ts]] - `imports_from` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/API_Middleware_auth/quota/rate-limit