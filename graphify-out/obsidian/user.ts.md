---
source_file: "apps/api/src/services/subscription/user.ts"
type: "code"
community: "subscription"
location: "L1"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/subscription
---

# user.ts

## Connections
- [[AppError]] - `imports` [EXTRACTED]
- [[GENERATION_FEATURES]] - `imports` [EXTRACTED]
- [[PlanCode]] - `imports` [EXTRACTED]
- [[QuotaExceededError]] - `imports` [EXTRACTED]
- [[QuotaFeature]] - `imports` [EXTRACTED]
- [[SubscriptionView]] - `imports` [EXTRACTED]
- [[adminUpdateUserSubscription()]] - `contains` [EXTRACTED]
- [[assertIndividualPlan()]] - `imports` [EXTRACTED]
- [[assertQuota()]] - `contains` [EXTRACTED]
- [[assertTenantQuota()]] - `imports` [EXTRACTED]
- [[dayRange()]] - `imports` [EXTRACTED]
- [[error.middleware.ts]] - `imports_from` [EXTRACTED]
- [[formatSubscription()]] - `imports` [EXTRACTED]
- [[getFileLimitsForUser()]] - `contains` [EXTRACTED]
- [[getFreePlan()]] - `imports` [EXTRACTED]
- [[getGenerationCount()]] - `contains` [EXTRACTED]
- [[getPodcastCount()]] - `contains` [EXTRACTED]
- [[getSubscriptionForUser()]] - `contains` [EXTRACTED]
- [[getTutorMessageCount()]] - `contains` [EXTRACTED]
- [[getUploadCount()]] - `contains` [EXTRACTED]
- [[getUsageForPeriod()]] - `imports` [EXTRACTED]
- [[getUsageVsLimits()]] - `contains` [EXTRACTED]
- [[getVideoCount()]] - `contains` [EXTRACTED]
- [[index.ts_2]] - `imports_from` [EXTRACTED]
- [[parseLimits()]] - `imports` [EXTRACTED]
- [[prisma_2]] - `imports` [EXTRACTED]
- [[prisma.ts]] - `imports_from` [EXTRACTED]
- [[resolveUpgradePlanCode()]] - `contains` [EXTRACTED]
- [[shared.ts_2]] - `imports_from` [EXTRACTED]
- [[subscription.service.ts]] - `re_exports` [EXTRACTED]
- [[tenant.ts]] - `imports_from` [EXTRACTED]
- [[usage.service.ts]] - `imports_from` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/subscription