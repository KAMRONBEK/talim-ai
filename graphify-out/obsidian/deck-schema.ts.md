---
source_file: "apps/api/src/lib/deck-schema.ts"
type: "code"
community: "Slide Deck Schema"
location: "L1"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/Slide_Deck_Schema
---

# deck-schema.ts

## Connections
- [[BODY]] - `contains` [EXTRACTED]
- [[Deck]] - `imports` [EXTRACTED]
- [[DeckSlide]] - `imports` [EXTRACTED]
- [[ParsedDeck]] - `contains` [EXTRACTED]
- [[ParsedSlide]] - `contains` [EXTRACTED]
- [[TITLE]] - `contains` [EXTRACTED]
- [[_deckAssign]] - `contains` [EXTRACTED]
- [[_slideAssign]] - `contains` [EXTRACTED]
- [[accentEnum]] - `contains` [EXTRACTED]
- [[base]] - `contains` [EXTRACTED]
- [[bullet]] - `contains` [EXTRACTED]
- [[chartSchema]] - `contains` [EXTRACTED]
- [[deckSchema]] - `contains` [EXTRACTED]
- [[iconEnum]] - `contains` [EXTRACTED]
- [[index.ts_2]] - `imports_from` [EXTRACTED]
- [[mermaidSchema]] - `contains` [EXTRACTED]
- [[slideSchema]] - `contains` [EXTRACTED]
- [[slides.service.ts]] - `imports_from` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/Slide_Deck_Schema