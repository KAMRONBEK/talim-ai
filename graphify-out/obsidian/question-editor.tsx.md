---
source_file: "apps/web/components/tenant/question-editor.tsx"
type: "code"
community: "Tenant Assessment Builder UI"
location: "L1"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/Tenant_Assessment_Builder_UI
---

# question-editor.tsx

## Connections
- [[BankQuestion]] - `imports` [EXTRACTED]
- [[Button]] - `imports` [EXTRACTED]
- [[CHOICE_TYPES]] - `contains` [EXTRACTED]
- [[Dialog()]] - `imports` [EXTRACTED]
- [[DialogContent()]] - `imports` [EXTRACTED]
- [[DialogHeader()]] - `imports` [EXTRACTED]
- [[DialogTitle()]] - `imports` [EXTRACTED]
- [[Input]] - `imports` [EXTRACTED]
- [[Label()]] - `imports` [EXTRACTED]
- [[QUESTION_TYPES]] - `contains` [EXTRACTED]
- [[QUESTION_TYPE_LABEL_KEYS]] - `contains` [EXTRACTED]
- [[QuestionBody]] - `contains` [EXTRACTED]
- [[QuestionEditor()]] - `contains` [EXTRACTED]
- [[QuestionType]] - `imports` [EXTRACTED]
- [[SIMPLE_ANSWER_TYPES]] - `contains` [EXTRACTED]
- [[StringListField()]] - `contains` [EXTRACTED]
- [[asStringArray()]] - `contains` [EXTRACTED]
- [[index.ts_2]] - `imports_from` [EXTRACTED]
- [[index.ts_3]] - `imports_from` [EXTRACTED]
- [[mutErr()_1]] - `contains` [EXTRACTED]
- [[page.tsx_19]] - `imports_from` [EXTRACTED]
- [[useAssessments.ts]] - `imports_from` [EXTRACTED]
- [[useCreateBankQuestion()]] - `imports` [EXTRACTED]
- [[usePatchBankQuestion()]] - `imports` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/Tenant_Assessment_Builder_UI