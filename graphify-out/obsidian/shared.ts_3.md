---
source_file: "apps/api/src/services/tenant/shared.ts"
type: "code"
community: "Tenant Service & Assignments"
location: "L1"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/Tenant_Service__Assignments
---

# shared.ts

## Connections
- [[AppError]] - `imports` [EXTRACTED]
- [[assignmentSchema]] - `contains` [EXTRACTED]
- [[assignments.ts]] - `imports_from` [EXTRACTED]
- [[createStudentSchema]] - `contains` [EXTRACTED]
- [[error.middleware.ts]] - `imports_from` [EXTRACTED]
- [[formatStudentRow()]] - `contains` [EXTRACTED]
- [[formatTenant()]] - `contains` [EXTRACTED]
- [[generateUniqueJoinCode()]] - `contains` [EXTRACTED]
- [[getDefaultTenantPlanId()]] - `contains` [EXTRACTED]
- [[organization.ts]] - `imports_from` [EXTRACTED]
- [[patchStudentSchema]] - `contains` [EXTRACTED]
- [[patchTenantSchema]] - `contains` [EXTRACTED]
- [[prisma_2]] - `imports` [EXTRACTED]
- [[prisma.ts]] - `imports_from` [EXTRACTED]
- [[randomJoinCode()]] - `contains` [EXTRACTED]
- [[slugifyOrgName()]] - `imports` [EXTRACTED]
- [[students.ts]] - `imports_from` [EXTRACTED]
- [[tenant-slug.ts]] - `imports_from` [EXTRACTED]
- [[tenant.service.ts]] - `re_exports` [EXTRACTED]
- [[uniqueSlug()]] - `contains` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/Tenant_Service__Assignments