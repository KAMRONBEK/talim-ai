---
source_file: "apps/api/src/services/mastery.service.ts"
type: "code"
community: "Community 92"
location: "L1"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/Community_92
---

# mastery.service.ts

## Connections
- [[AppLocale]] - `imports` [EXTRACTED]
- [[ClassMastery]] - `imports` [EXTRACTED]
- [[LearnerMastery]] - `contains` [EXTRACTED]
- [[MasteryTopic]] - `imports` [EXTRACTED]
- [[contentAccess.service.ts]] - `imports_from` [EXTRACTED]
- [[getAssignedContentIds()]] - `imports` [EXTRACTED]
- [[getClassMastery()]] - `contains` [EXTRACTED]
- [[getLearnerMastery()]] - `contains` [EXTRACTED]
- [[index.ts_2]] - `imports_from` [EXTRACTED]
- [[learningProgress.service.ts]] - `imports_from` [EXTRACTED]
- [[prisma_2]] - `imports` [EXTRACTED]
- [[prisma.ts]] - `imports_from` [EXTRACTED]
- [[progress.ts]] - `imports_from` [EXTRACTED]
- [[resolveSectionTitles()]] - `contains` [EXTRACTED]
- [[resolveTenantIdForUser()]] - `imports` [EXTRACTED]
- [[roundPct()]] - `contains` [EXTRACTED]
- [[tenant.controller.ts]] - `imports_from` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/Community_92