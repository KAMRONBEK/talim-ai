---
source_file: "package.json"
type: "code"
community: "Database Scripts & Commands"
location: "L4"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/Database_Scripts__Commands
---

# scripts

## Connections
- [[build_3]] - `contains` [EXTRACTED]
- [[create-admin_1]] - `contains` [EXTRACTED]
- [[create-tenant-owner_1]] - `contains` [EXTRACTED]
- [[dbgenerate_1]] - `contains` [EXTRACTED]
- [[dbmigrate_1]] - `contains` [EXTRACTED]
- [[dbmigratedeploy]] - `contains` [EXTRACTED]
- [[dbpush_1]] - `contains` [EXTRACTED]
- [[dbseed_1]] - `contains` [EXTRACTED]
- [[dbstudio_1]] - `contains` [EXTRACTED]
- [[dev_3]] - `contains` [EXTRACTED]
- [[devadmin]] - `contains` [EXTRACTED]
- [[devall]] - `contains` [EXTRACTED]
- [[devclean]] - `contains` [EXTRACTED]
- [[devinfra]] - `contains` [EXTRACTED]
- [[dockerdown]] - `contains` [EXTRACTED]
- [[dockerre]] - `contains` [EXTRACTED]
- [[dockerup]] - `contains` [EXTRACTED]
- [[dopplersetup]] - `contains` [EXTRACTED]
- [[dopplerupload]] - `contains` [EXTRACTED]
- [[format]] - `contains` [EXTRACTED]
- [[inspect-chunks_1]] - `contains` [EXTRACTED]
- [[lint_3]] - `contains` [EXTRACTED]
- [[package.json_3]] - `contains` [EXTRACTED]
- [[qafixtures]] - `contains` [EXTRACTED]
- [[qaovernight]] - `contains` [EXTRACTED]
- [[qapreflight]] - `contains` [EXTRACTED]
- [[typecheck_3]] - `contains` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/Database_Scripts__Commands