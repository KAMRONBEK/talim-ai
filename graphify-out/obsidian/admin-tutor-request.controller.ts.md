---
source_file: "apps/api/src/controllers/admin-tutor-request.controller.ts"
type: "code"
community: "Community None"
location: "L1"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/Community_None
---

# admin-tutor-request.controller.ts

## Connections
- [[AppError]] - `imports` [EXTRACTED]
- [[AuthenticatedRequest]] - `imports` [EXTRACTED]
- [[admin.routes.ts]] - `imports_from` [EXTRACTED]
- [[approveTutorRequest()]] - `contains` [EXTRACTED]
- [[audit.service.ts]] - `imports_from` [EXTRACTED]
- [[auth.middleware.ts]] - `imports_from` [EXTRACTED]
- [[error.middleware.ts]] - `imports_from` [EXTRACTED]
- [[getParam()]] - `imports` [EXTRACTED]
- [[listSchema]] - `contains` [EXTRACTED]
- [[listTutorRequests()]] - `contains` [EXTRACTED]
- [[params.ts]] - `imports_from` [EXTRACTED]
- [[rejectTutorRequest()]] - `contains` [EXTRACTED]
- [[tutorRequest.service.ts]] - `imports_from` [EXTRACTED]
- [[writeAdminAuditLog()]] - `imports` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/Community_None